...
const {createYahooLatestNewsDiscoveryService} = require('./yahoo-latest-news-discovery');
const {
...
function createAnalysisPackageRuntime({
  ...
  const yahooLatestNewsDiscovery = createYahooLatestNewsDiscoveryService({fetchImpl});
  const yahooCurrentNewsArticleContentAcquisition =
    createYahooCurrentNewsArticleContentAcquisitionService({fetchImpl});
