...
const FIVE_SESSION_SNAPSHOT_KEYS = Object.freeze([
  'market',
  'symbol',
  'instrumentName',
  'instrumentType',
...
function createFiveSessionSnapshot(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new TypeError('Invalid five-session snapshot: input must be an object');
  }
  const forbidden = ['exchangeTimezone', 'primaryCompletedSessionDate', 'completeness'];
  for (const field of forbidden) {
    if (Object.prototype.hasOwnProperty.call(input, field)) {
      throw new TypeError(`Invalid five-session snapshot: ${field} is derived and must not be supplied`);
    }
  }
  const market = canonicalMarket(input.market);
  ...
  if (!market || !symbol || !instrumentName || !instrumentType
    ...
    throw new TypeError('Invalid five-session snapshot: invalid identity or market state');
  }
  if (!Array.isArray(input.completedSessions) || input.completedSessions.length > 5) {
    throw new TypeError('Invalid five-session snapshot: completedSessions must contain zero to five sessions');
  }
  const completedSessions = input.completedSessions.map(record => {
    const validation = validateCompletedRegularSession(record, market);
    if (!validation.valid) throw new TypeError('Invalid five-session snapshot: invalid completed session');
  ...
  for (let index = 1; index < completedSessions.length; index += 1) {
    if (completedSessions[index - 1].sessionDate >= completedSessions[index].sessionDate) {
      throw new TypeError('Invalid five-session snapshot: completed sessions must be oldest to newest');
    }
    if (completedSessions[index].previousClose !== completedSessions[index - 1].close) {
      throw new TypeError('Invalid five-session snapshot: completed session previousClose chain is inconsistent');
    }
  }
  ...
  if (input.currentOverlay !== null) {
    ...
    if (!validation.valid || input.currentOverlay.marketState !== marketState) {
      throw new TypeError('Invalid five-session snapshot: invalid current overlay');
    }
    if (completedSessions.length
        && input.currentOverlay.referenceClose !== completedSessions[completedSessions.length - 1].close) {
      throw new TypeError('Invalid five-session snapshot: current overlay referenceClose must match latest completed close');
    }
    if (completedSessions.length
        && input.currentOverlay.sessionDate < completedSessions[completedSessions.length - 1].sessionDate) {
      throw new TypeError('Invalid five-session snapshot: current overlay cannot predate latest completed session');
    }
  ...
  return deepFreeze({
    ...
    completeness: count === 5 ? 'COMPLETE' : count ? 'PARTIAL' : 'UNAVAILABLE',
    completedSessions,
    currentOverlay
  });
}

function validateFiveSessionSnapshot(snapshot) {
  if (!hasExactKeys(snapshot, FIVE_SESSION_SNAPSHOT_KEYS)) {
    return deepFreeze({valid: false, errors: ['invalid canonical property shape or order']});
  }
  try {
    const expected = createFiveSessionSnapshot({
