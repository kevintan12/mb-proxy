...
const CNBC_ARTICLE_ACQUISITION_DIAGNOSTIC_FAILURE_TYPES = Object.freeze([
  'INVALID_INPUT',
  'TIMEOUT',
  'NETWORK_FAILURE',
  'HTTP_FAILURE',
...
const SAFE_ARTICLE_FAILURE_TYPES = new Set(CNBC_ARTICLE_ACQUISITION_DIAGNOSTIC_FAILURE_TYPES);
const SAFE_SIZE_FAILURE_TYPES = new Set([
  'TITLE_TOO_LARGE',
  'RESPONSE_TOO_LARGE',
  'ARTICLE_TEXT_TOO_LARGE',
  'RESULT_TOO_LARGE'
]);
const SAFE_EXTRACTION_FAILURE_TYPES = new Set(CNBC_EXTRACTION_DIAGNOSTIC_FAILURE_TYPES);
...
function emitFailureDiagnostic(
  ...
  try {
    ...
    if (failureType === 'CONTENT_TOO_LARGE') {
      diagnostic.sizeFailureType = SAFE_SIZE_FAILURE_TYPES.has(sizeFailureType)
        ? sizeFailureType
        : 'UNKNOWN_SIZE_FAILURE';
    }
    if (failureType === 'EXTRACTION_FAILURE') {
      diagnostic.extractionFailureType = SAFE_EXTRACTION_FAILURE_TYPES.has(extractionFailureType)
        ? extractionFailureType
        : 'UNKNOWN_EXTRACTION_FAILURE';
    }
    onDiagnostics(deepFreeze(diagnostic));
  } catch (error) {
    // Observability must not affect retrieval behavior.
  }
}
...
function createCnbcSelectedArticleRetrievalOrchestrationService({
  ...
  if (onDiagnostics !== undefined && typeof onDiagnostics !== 'function') {
    throw new TypeError('onDiagnostics must be a function');
  }
      ...
      for (const selection of canonicalSelections) {
        ...
        try {
          ...
          emitFailureDiagnostic(
            onDiagnostics,
            candidate.reference,
            SAFE_ARTICLE_FAILURE_TYPES.has(error?.code) ? error.code : 'UNKNOWN_FAILURE',
            error?.sizeFailureType,
        ...
        try {
          ...
          emitFailureDiagnostic(onDiagnostics, candidate.reference, 'ARTICLE_CONTRACT_FAILURE');
          return failure(
            'ARTICLE_CONTRACT_FAILURE',
