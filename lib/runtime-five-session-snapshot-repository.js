...
} = require('./postgres-five-session-snapshot-repository');
...
class RuntimeSnapshotPersistenceError extends Error {
  constructor(code, message, diagnosticDetails = null) {
    super(message);
    this.name = 'RuntimeSnapshotPersistenceError';
    this.code = code;
    if (diagnosticDetails) this.diagnosticDetails = Object.freeze({...diagnosticDetails});
  }
}

const CANONICAL_SESSION_DIFFERENCE_FIELDS = Object.freeze([
  'open',
  'high',
  'low',
  'close',
...
function differingCanonicalSessionFields(stored, acquired) {
  return Object.freeze(CANONICAL_SESSION_DIFFERENCE_FIELDS.filter(field =>
    JSON.stringify(stored[field]) !== JSON.stringify(acquired[field])));
}

function createRuntimeFiveSessionSnapshotRepository({
  repository,
  runtime,
  ...
  const resolvedRepository = repository || createPostgresFiveSessionSnapshotRepository(
    runtime || getPostgresRuntime()
  );
  if (!resolvedRepository || typeof resolvedRepository.upsertSnapshot !== 'function') {
    throw new TypeError('repository must provide upsertSnapshot');
  }
  ...
  return Object.freeze({
    async persistSnapshot(snapshot) {
      ...
      if (!validation.valid) {
        throw new TypeError('Invalid canonical five-session snapshot');
      }
      try {
        const persisted = await resolvedRepository.upsertSnapshot({
        ...
        if (acquiredNewest && persistedNewest
            && persistedNewest.sessionDate !== acquiredNewest.sessionDate) {
          const mismatch = new RuntimeSnapshotPersistenceError(
            'SNAPSHOT_READ_MISMATCH',
            'Persisted snapshot does not match the acquired snapshot',
            {
              mismatchReason: 'NEWEST_SESSION_DATE',
              acquiredNewestSessionDate: acquiredNewest.sessionDate,
              persistedNewestSessionDate: persistedNewest.sessionDate
            }
          );
          if (persistedNewest.sessionDate > acquiredNewest.sessionDate) {
            try {
              mismatch.persistedSnapshot = createFiveSessionSnapshot({
                market: snapshot.market,
                symbol: snapshot.symbol,
                ...
                completedSessions: persisted,
                currentOverlay: null
              });
            } catch (error) {
              // A persisted candidate is never promoted unless it is canonical.
            }
          }
          throw mismatch;
        }
          ...
          if (!stored) {
            throw new RuntimeSnapshotPersistenceError(
              ...
              'Persisted snapshot does not match the acquired snapshot',
              {
                ...
                missingSessionDate: session.sessionDate
              }
            );
          }
          if (canonicalSessionSignature(stored) !== canonicalSessionSignature(session)) {
            throw new RuntimeSnapshotPersistenceError(
              'SNAPSHOT_READ_MISMATCH',
              'Persisted snapshot does not match the acquired snapshot',
              {
                ...
                differingFields: differingCanonicalSessionFields(stored, session)
              }
            );
          }
          return stored;
        ...
        throw new RuntimeSnapshotPersistenceError(
          ...
          'Five-session snapshot persistence failed'
        );
      }
    }
  });
