...
const CNBC_RECAP_URL = /^https:\/\/www\.cnbc\.com\/\d{4}\/\d{2}\/\d{2}\/stock-market-today-live-updates\.html$/;
...
function normalizeInput(input) {
  ...
  const hasEditorialPublisher = Object.prototype.hasOwnProperty.call(input, 'publisher');
  const permitsEditorialPublisher = source?.id === YAHOO_RECAP_SOURCE_ID
    && evidenceCategory === YAHOO_RECAP_CATEGORY;
  let publisher = source?.provenance.publisher || null;
  if (permitsEditorialPublisher) {
    if (!hasEditorialPublisher || typeof input.publisher !== 'string'
        || !input.publisher.trim() || input.publisher !== input.publisher.trim()) {
      errors.push('valid provider-owned publisher is required');
    } else {
      publisher = input.publisher;
    }
  } else if (hasEditorialPublisher) {
    errors.push('publisher override is not permitted');
  }
  ...
  const unknownCnbcRecapPublication = input.publishedAt === null
    && source?.id === 'us.cnbc' && market === 'US' && evidenceCategory === 'news'
    && CNBC_RECAP_URL.test(canonicalUrl || '');
  if (!publishedAt && !unknownCnbcRecapPublication) {
    errors.push('publishedAt must be a valid ISO-8601 timestamp with timezone');
  }
...
function validateEvidenceItem(item) {
  ...
  try {
    ...
    if (item.sourceId === YAHOO_RECAP_SOURCE_ID
        && item.evidenceCategory === YAHOO_RECAP_CATEGORY
        && typeof item.provenance?.publisher === 'string') {
      reconstruction.publisher = item.provenance.publisher;
    }
    expected = createEvidenceItem(reconstruction);
  } catch (error) {
