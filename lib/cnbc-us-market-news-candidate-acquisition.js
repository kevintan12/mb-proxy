...
const UNSUPPORTED_BIGGEST_MOVES_PATH =
  /^\/\d{4}\/\d{2}\/\d{2}\/stocks-making-the-biggest-moves-(?:midday|premarket|after-hours)-[^/]+\.html$/;
...
function isUnsupportedBiggestMovesUrl(value) {
  try {
    return UNSUPPORTED_BIGGEST_MOVES_PATH.test(new URL(value).pathname);
  } catch (error) {
    return false;
  }
}
...
function candidateInput(providerItem, horizon, reference) {
  ...
  if (!title || !canonicalUrl || isUnsupportedBiggestMovesUrl(canonicalUrl)
      || !publishedAt || !horizon) return null;
  return {
    reference,
    horizon: {
      classification: horizon.classification,
