...
function activeSessionEvidenceWindowResult({marketState, generatedAt, benchmarkSnapshots = []} = {}) {
  ...
  if (presentOverlays.length === 0) {
    return deepFreeze({window: null, failureType: 'ACTIVE_WINDOW_UNAVAILABLE'});
  }
  if (presentOverlays.some(overlay => overlay.marketState !== marketState
      || overlay.sessionDate !== current.exchangeDate)) {
  ...
  if (preMarket.session !== 'PRE' || !preMarket.sessionStartTime
      ...
      || Date.parse(preMarket.sessionStartTime) > generated.getTime()) {
    return deepFreeze({window: null, failureType: 'ACTIVE_WINDOW_UNAVAILABLE'});
  }

  return deepFreeze({window: deepFreeze({
