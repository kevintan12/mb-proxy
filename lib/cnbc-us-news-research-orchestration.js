  ...
  return Object.freeze({
    async researchNews({targetSessionDate, horizons} = {}) {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
          acquired = await candidateAcquisition.acquireCandidates({
        ...
        try {
          constructionResult = await evidenceConstruction.constructProvisionalEvidence({
            candidateCollection,
            retrievedArticles,
          ...
          if (attempt === 0
              && typeof candidateAcquisition.evictValidatedCacheHits === 'function'
              && candidateAcquisition.evictValidatedCacheHits(
                acquired,
                constructionResult?.failedCandidateReferences
              )) {
            continue;
          }
          return finish(failure(
        ...
        if (typeof candidateAcquisition.completeValidatedDiscoveries === 'function') {
          try {
            candidateAcquisition.completeValidatedDiscoveries(acquired);
          } catch (cacheError) {
            // Warm-runtime caching cannot alter research correctness.
          }
        }
