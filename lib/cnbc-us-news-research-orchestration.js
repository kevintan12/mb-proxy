    ...
    async researchNews({targetSessionDate, horizons} = {}) {
      ...
      try {
        const retainedByReference = new Map(
          retainedArticles.map(article => [article.reference, article])
        );
        const retrievalResult = {
          ok: true, type: 'SUCCESS', candidateCollection, selections,
          retrievedArticles: selections.filter(selection => selection.decision === 'USE')
            .map(selection => retainedByReference.get(selection.reference))
        };
