...
function failure(type, message, extractionFailureType = null) {
  const result = {ok: false, type, message, articleContent: null};
  if (extractionFailureType !== null) result.extractionFailureType = extractionFailureType;
...
function headlineWords(value) {
  ...
  return normalized ? normalized.toLowerCase().normalize('NFKC')
    .replace(/[^\p{L}\p{N}]+/gu, ' ').trim().split(/\s+/).filter(Boolean) : [];
}

function compatibleHeadline(actual, requested, strongIdentity) {
  if (requested === null) return true;
  const actualWords = headlineWords(actual);
  const requestedWords = headlineWords(requested);
  if (!actualWords.length || !requestedWords.length) return false;
  if (actualWords.join(' ') === requestedWords.join(' ')) return true;
  if (!strongIdentity) return false;
  const actualSet = new Set(actualWords);
  const shared = requestedWords.filter(word => actualSet.has(word)).length;
  return shared >= 3 && shared / Math.min(actualWords.length, requestedWords.length) >= 0.8;
}
...
function extract(html, requestedUrl, requestedHeadline) {
  ...
  const articleNodes = nodes.filter(isArticle);
  if (articleNodes.length === 0) {
    return {articleContent: null, failureType: 'NO_COMPATIBLE_ARTICLE_METADATA'};
  }
  const requestedIdentity = identity(requestedUrl);
  const pageCanonical = canonicalLink(html);
  if (pageCanonical && identity(pageCanonical) !== requestedIdentity) {
    return {articleContent: null, failureType: 'CANONICAL_OR_REDIRECT_MISMATCH'};
  }
  const metadataArticles = articleNodes.map(article => ({article, url: metadataUrl(article)}));
  if (!metadataArticles.some(entry => entry.url)) {
    return {articleContent: null, failureType: 'NO_COMPATIBLE_ARTICLE_METADATA'};
  }
  const articles = metadataArticles.filter(({url}) =>
    url && identity(url) === requestedIdentity
  );
  if (articles.length === 0) {
    return {articleContent: null, failureType: 'ARTICLE_IDENTITY_OR_HEADLINE_MISMATCH'};
  }
  let identityOrHeadlineMismatch = false;
  let bodyUnavailable = false;
  for (const article of articles) {
    const url = article.url || pageCanonical;
    const headline = plainText(article.article.headline);
    const strongIdentity = Boolean(pageCanonical
      && identity(pageCanonical) === requestedIdentity
      && identity(url) === requestedIdentity);
    if (!url || !headline || Buffer.byteLength(headline, 'utf8') > MAX_HEADLINE_BYTES
        || !compatibleHeadline(headline, requestedHeadline, strongIdentity)) {
      identityOrHeadlineMismatch = true;
      continue;
    }
    const articleText = plainText(article.article.articleBody) || renderedArticleBody(html);
    if (!articleText) {
    ...
    return {articleContent: {
      ...
      headline,
      publisher: publisherName(article.article.publisher),
      publishedAt: timestamp(article.article.datePublished),
      updatedAt: timestamp(article.article.dateModified),
      articleText
    }, failureType: null};
  }
  return {
    articleContent: null,
    failureType: bodyUnavailable ? 'NO_ARTICLE_BODY_CONTAINER_OR_TEXT'
      : identityOrHeadlineMismatch ? 'ARTICLE_IDENTITY_OR_HEADLINE_MISMATCH'
  ...
  return deepFreeze({
    async acquireArticleContent({url, headline = null} = {}) {
      ...
      if (!extracted.articleContent) {
        return failure('NO_USABLE_ARTICLE', 'Yahoo current-news article has no usable provider-owned content',
          extracted.failureType);
      }
      if (Buffer.byteLength(extracted.articleContent.articleText, 'utf8') > maxArticleTextBytes) {
        return failure('ARTICLE_TEXT_TOO_LARGE', 'Yahoo current-news article text exceeds configured bounds');
      }
      const articleContent = deepFreeze({sourceId: 'us.yahoo-finance', ...extracted.articleContent});
