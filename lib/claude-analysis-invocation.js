...
async function invokeClaudeAnalysis({
  ...
  try {
    const canonicalInput = canonicalInputCopy(input);
    const normalized = normalizeEvidenceLimitedStatus(parsed, canonicalInput);
    const output = createClaudeAnalysisOutput(withDerivedEvidenceReferences(normalized), canonicalInput);
