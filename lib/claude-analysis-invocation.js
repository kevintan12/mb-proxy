...
const PROVIDER_SECTION_TRANSPORT_KEY = 'sectionsById';
const PROVIDER_SECTION_PAYLOAD_KEYS = Object.freeze([
...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  `Return the ${REPORT_HEADER} using sectionsById with exactly these canonical section keys: ${REPORT_SECTION_NAMES.join('; ')}. MarketBrief converts those keys into the final ordered report sections.`,
  ...
  'In sectionsById, the FURTHER READINGS payload must be exactly {"content":null,"evidenceRefs":[],"telemetryRefs":[],"uncertainties":[]}. MarketBrief resolves and renders Further Readings separately; do not put explanatory text, URLs, provenance, or evidence references inside Section 8.',
...
function providerCompatibleSchema(value) {
  ...
  if (result.type === 'object' && !Object.hasOwn(result, 'additionalProperties')) {
    result.additionalProperties = false;
  }
  return result;
}

const PROVIDER_SECTION_PAYLOAD_SCHEMA = deepFreeze({
  // Anthropic enforces the exact keyed eight-section envelope.  The existing
  // deterministic transport conversion and canonical validator enforce each
  // payload's exact fields and reference syntax after parsing, without
  // multiplying that grammar eight times in the provider request.
  type: 'object',
  additionalProperties: false
});

const providerOutputSchema = providerCompatibleSchema(CLAUDE_ANALYSIS_OUTPUT_JSON_SCHEMA);
delete providerOutputSchema.properties.sections;
const CLAUDE_ANALYSIS_PROVIDER_JSON_SCHEMA = deepFreeze({
  ...providerOutputSchema,
  required: CLAUDE_ANALYSIS_OUTPUT_JSON_SCHEMA.required.map(key =>
    key === 'sections' ? PROVIDER_SECTION_TRANSPORT_KEY : key),
  properties: {
    ...providerOutputSchema.properties,
    [PROVIDER_SECTION_TRANSPORT_KEY]: {
      type: 'object',
      additionalProperties: false,
      required: REPORT_SECTION_NAMES.slice(),
      properties: Object.fromEntries(REPORT_SECTION_NAMES.map(name => [name, PROVIDER_SECTION_PAYLOAD_SCHEMA]))
    }
  }
});
...
function activeSessionInstruction(modelInput) {
  ...
  return 'ACTIVE_SESSION request-specific semantics: currentSessionContext contains the exact '
    ...
    + 'Section 1 may use natural current-first wording, must cite at least one CURRENT_SESSION '
    + 'evidence reference in Section 1 evidenceRefs regardless of array position, must not lead with '
    + 'the prior completed session, and must use '
...
function providerStructureDiagnostic(value) {
  const sectionsById = value && typeof value === 'object' && !Array.isArray(value)
    ? value[PROVIDER_SECTION_TRANSPORT_KEY] : null;
  const isObject = sectionsById && typeof sectionsById === 'object' && !Array.isArray(sectionsById);
  const suppliedKeys = isObject ? Reflect.ownKeys(sectionsById) : [];
  const presentIds = REPORT_SECTION_NAMES.filter(name => suppliedKeys.includes(name));
  const unknownIdCount = suppliedKeys.filter(key =>
    typeof key !== 'string' || !REPORT_SECTION_NAMES.includes(key)).length;
  const malformedSectionCount = isObject ? presentIds.filter(name => {
    const section = sectionsById[name];
    if (name === REPORT_SECTION_NAMES.length - 1) {
      return !section || typeof section !== 'object' || Array.isArray(section);
    }
    return !hasExactKeySet(section, PROVIDER_SECTION_PAYLOAD_KEYS);
  }).length : 0;
  return deepFreeze({
    stage: 'claudeAnalysisStructureNormalization',
    suppliedStructureType: Array.isArray(sectionsById) ? 'ARRAY'
      : isObject ? 'OBJECT' : sectionsById === null ? 'NULL_OR_MISSING' : typeof sectionsById,
    suppliedSectionCount: isObject ? suppliedKeys.length
      : Array.isArray(sectionsById) ? sectionsById.length : null,
    canonicalSectionIdsPresent: presentIds,
    missingCanonicalSectionIds: REPORT_SECTION_NAMES.filter(name => !presentIds.includes(name)),
    unknownSectionIdCount: unknownIdCount,
...
function convertProviderSections(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new TypeError('Claude structured result has invalid provider transport shape');
  }
  const expectedKeys = CLAUDE_ANALYSIS_OUTPUT_KEYS.map(key =>
    key === 'sections' ? PROVIDER_SECTION_TRANSPORT_KEY : key);
  if (!hasExactKeySet(value, expectedKeys)) {
    throw new TypeError('Claude structured result has invalid provider transport shape');
  }
  const sectionsById = value[PROVIDER_SECTION_TRANSPORT_KEY];
  if (!hasExactKeySet(sectionsById, REPORT_SECTION_NAMES)) {
    throw new TypeError('Claude structured result has invalid provider section identifiers');
  }
  const sections = REPORT_SECTION_NAMES.map((name, index) => {
    const payload = sectionsById[name];
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
      throw new TypeError('Claude structured result has malformed section payload');
    }
    // Further Readings is always owned and resolved by MarketBrief after synthesis.
    if (index === REPORT_SECTION_NAMES.length - 1) {
      return {name, content: null, evidenceRefs: [], telemetryRefs: [], uncertainties: []};
    }
    if (!hasExactKeySet(payload, PROVIDER_SECTION_PAYLOAD_KEYS)) {
      throw new TypeError('Claude structured result has malformed section payload');
    }
    return {
      name,
      ...
      telemetryRefs: payload.telemetryRefs,
  ...
  return {
    ...
    furtherReadings: value.furtherReadings,
...
function repairSingleActiveCurrentSessionCitation(output, input) {
  ...
  if (currentSessionReferences.size !== 1 || !Array.isArray(output?.sections)) return result;
  ...
  if (summary.evidenceRefs.includes(reference)) return result;
...
function normalizeEvidenceLimitedStatus(output, input) {
  ...
  const summary = output.sections[0];
  const failedWithSupportedSummary = output.status === 'FAILED'
    && normalizableSection(summary, 0) && summary.content !== null
    && summary.evidenceRefs.length > 0;
  if (output.status === 'FAILED' && !failedWithSupportedSummary) return output;
  const workingOutput = failedWithSupportedSummary ? {...output, status: 'DEGRADED'} : output;
...
const GENERATED_CITATION_GAPS = Object.freeze({
  ...
  referenceIntegrity: 'Supported analysis could not be validated from the generated reference set.',
...
function normalizeDynamicReferenceViolations(output, input) {
  ...
  if (activeWithCurrentEvidence) {
    for (let index = 1; index < REPORT_SECTION_NAMES.length - 1; index++) {
      const section = sections[index];
      if (!normalizableSection(section, index) || section.content === null) continue;
      const unknownEvidenceCount = section.evidenceRefs.filter(reference => !allEvidence.has(reference)).length;
      const unknownTelemetryCount = section.telemetryRefs.filter(reference => !allTelemetry.has(reference)).length;
      if (unknownEvidenceCount || unknownTelemetryCount) {
        events.push({
          ...
          allowedReferenceCount: allEvidence.size + allTelemetry.size,
          offendingReferenceCount: unknownEvidenceCount + unknownTelemetryCount
        });
        localize(index, GENERATED_CITATION_GAPS.referenceIntegrity);
      }
    }
  }
  ...
  if (normalizableSection(causalSection, 1) && causalSection.content !== null
      ...
      && !causalSection.evidenceRefs.some(reference => currentDriverReferences.has(reference))) {
    events.push({
  ...
  if (normalizableSection(normalizedCausalSection, 1) && normalizedCausalSection.content !== null
      ...
      && hasDirectMarketCausalClaim(normalizedCausalSection.content)
      && !activeWithCurrentEvidence
      && !normalizedCausalSection.evidenceRefs.some(reference => principalCatalysts.has(reference))) {
    events.push({
      stage: 'claudeAnalysisSectionNormalization', sectionIndex: 1,
      violationCategory: 'MISSING_PRINCIPAL_CATALYST',
      suppliedReferenceCount: normalizedCausalSection.evidenceRefs.length,
      allowedReferenceCount: principalCatalysts.size,
      offendingReferenceCount: normalizedCausalSection.evidenceRefs.length
    });
    localize(1, GENERATED_CITATION_GAPS.causality);
  }
  ...
  if (normalizableSection(sections[1], 1) && sections[1].content !== null
      ...
      && sections[1].telemetryRefs.every(reference => allTelemetry.has(reference))
      && activeWithCurrentEvidence
      && hasDirectMarketCausalClaim(sections[1].content)
      && !hasPriorCompletedSessionCausalClaim(sections[1].content, input)
      && !sections[1].evidenceRefs.some(reference => currentSessionPrincipalCatalysts.has(reference))) {
    events.push({
      stage: 'claudeAnalysisSectionNormalization', sectionIndex: 1,
      violationCategory: 'MISSING_CURRENT_SESSION_PRINCIPAL_CATALYST',
      suppliedReferenceCount: sections[1].evidenceRefs.length,
      allowedReferenceCount: currentSessionPrincipalCatalysts.size,
      offendingReferenceCount: sections[1].evidenceRefs.length
    });
    localize(1, GENERATED_CITATION_GAPS.causality);
  }
  ...
  if (activeWithCurrentEvidence) {
    for (let index = 2; index < REPORT_SECTION_NAMES.length - 1; index++) {
      const section = sections[index];
      if (!normalizableSection(section, index) || section.content === null
          ...
          || !hasDirectMarketCausalClaim(section.content)) continue;
      const priorClaim = hasPriorCompletedSessionCausalClaim(section.content, input);
      const allowed = priorClaim
        ? completedSessionPrincipalCatalysts : currentSessionPrincipalCatalysts;
      if (!section.evidenceRefs.some(reference => allowed.has(reference))) {
        events.push({
          stage: 'claudeAnalysisSectionNormalization', sectionIndex: index,
          violationCategory: priorClaim
...
async function invokeClaudeAnalysis({
  ...
  try {
    ...
    const transportOutput = convertProviderSections(parsed);
    ...
    try { emitPreNormalizationSectionDiagnostics(onDiagnostics, canonicalizedReferences, canonicalInput); }
    catch (error) { /* Diagnostics cannot alter analysis behavior. */ }
    const citationRepair = repairSingleActiveCurrentSessionCitation(
      canonicalizedReferences, canonicalInput
    );
