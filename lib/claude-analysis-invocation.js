...
const INCOMPLETE_US_REPORT_GAP =
  'The generated report did not establish complete analytical coverage.';
...
function activeUsIndependentSurvival(input) {
  return input.analysisRequest.selectedScope === 'US'
...
function isActiveUsRequest(input) {
  return input?.analysisRequest?.selectedScope === 'US'
    && input.marketPackages?.some(item => item.market === 'US'
      && US_ACTIVE_SESSION_STATES.includes(item.marketContext.marketState)) === true;
}
...
function safeSectionViolationCategories(errors, index) {
  ...
  add(joined.includes('analyst jargon'), 'PLAIN_LANGUAGE_VALIDATION');
  return categories;
}
...
function normalizeEvidenceLimitedStatus(output, input) {
  ...
  const independentSurvival = activeUsIndependentSurvival(input);
...
function assertUsReferenceIntegrity(output, input) {
  if (input.analysisRequest.selectedScope !== 'US') return;
...
function normalizeUsIndependentSections(output, input) {
  if (!activeUsIndependentSurvival(input)
  ...
  if (supportedSectionCount(working) === 0
    ...
    if (activeSession && sectionIndex === 2) {
      ...
      if (normalizableSection(section, 2) && section.content !== null
        ...
        const violations = [
          ...
          [scope.nonBenchmarkTelemetryCount, 'NON_BENCHMARK_TELEMETRY'],
...
async function invokeClaudeAnalysis({
  ...
  try {
    ...
    assertUsReferenceIntegrity(canonicalizedReferences, canonicalInput);
