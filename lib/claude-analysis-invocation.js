...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  ...
  'For Section 2 KEY MARKET DRIVERS, when a material macroeconomic release is supported by explicit current, consensus or expected, and previous comparable values in the supplied evidence, present that three-way comparison and explain both the surprise versus expectations and the change versus the previous reading. Use only values explicitly supplied in the package, do not invent any missing comparison value, do not force immaterial macro items into a three-number format, and do not repeat the same comparison unnecessarily across Sections 1, 2, and 3.',
  'For Section 3 WHAT DROVE / IS DRIVING THE MARKET, explain HOW supported principal catalysts transmitted into observed market behavior and cite at least one supplied evidence reference from evidenceContext.principalCatalysts. supportingEvidence may supplement but cannot substitute for a principal catalyst. Connect only supported catalysts to observed market moves and explain interactions among drivers where materially relevant. Do not duplicate Section 2. Temporal proximity alone is not causality. Preserve the completed-session and subsequent-development distinction: never present a SUBSEQUENT_DEVELOPMENT as causing an earlier completed-session move.',
  'For Section 4 STOCKS & SECTORS IN FOCUS, examine all materially relevant broad-market materialEvents, supportingEvidence, recap and general CNBC evidence, sessionAssociations, and applicable telemetry. Rank the significant companies and sectors supported by that material, covering broad-market leadership and laggards, notable individual movers, closing-session breadth, sector rotation, weakness, or unusual moves, explaining why each matters and comparing it with the broader market where useful. Do not produce a generic mover list. If the evidence is insufficient, use qualified analysis or the existing null/DEGRADED behavior rather than filling the section. Review evidenceContext.sessionAssociations and, when materially relevant and independently in broadMarketFocus, use each associated supplied evidence reference as current-session recap or context for marketContext.primaryCompletedSessionDate, even when that evidence is also a SUBSEQUENT_DEVELOPMENT. Never present post-close session-associated evidence as having caused the earlier completed-session move, and never treat session association as PRINCIPAL_CATALYST eligibility. Section 4 must remain broad-market and independent of My Stocks and Watchlist; membership in either list must not determine which broad-market movers Section 4 discusses. Do not require Section 4 to use every associated reference or use an association that is immaterial.',
  'For Section 4 STOCKS & SECTORS IN FOCUS, inspect evidenceContext.broadMarketFocus. A populated Section 4 must cite at least one broadMarketFocus evidenceRef and explicitly mention at least one validated subject belonging to each cited focus entry that it uses. Section 4 evidenceRefs may contain only broadMarketFocus references; telemetryRefs may contain only benchmark telemetry references for supported market or index comparisons, never portfolio or watchlist stock telemetry. Discuss a My Stocks or Watchlist company in Section 4 only when it is independently present as a validated COMPANY subject in broadMarketFocus and cite that company\'s focus reference; membership alone never qualifies it. Portfolio or watchlist telemetry, index-only commentary, generic sector extrapolation, recaps, supporting evidence, or session association alone cannot satisfy this requirement. Use only materially relevant focus entries; do not require every focus entry. Portfolio or watchlist membership neither qualifies nor disqualifies a focus subject.',
  'When Section 4 uses a session-associated broad-market evidence reference that is also in broadMarketFocus, cite that reference in Section 4 evidenceRefs. Do not copy such a reference into Section 5 merely because it is session-associated, broad-market evidence, or relevant to market leadership, sectors, movers, breadth, or rotation. Section 5 remains strictly limited to the initiating-list securities\' permitted telemetryRefs, permitted direct evidenceRefs, and permitted upcoming-event evidenceRefs; a reference may appear in Section 5 only when it independently satisfies those existing initiating-list eligibility rules.',
  'Hard output constraint for Sections 7-10: content must be either null or a non-empty already-trimmed string. Except when content is null, evidenceRefs must contain at least one valid supplied evidence reference; telemetryRefs alone never satisfy this grounding requirement. Every factual claim or qualified interpretation in a populated section must be grounded in its listed supplied evidenceRefs, with permitted telemetryRefs added for quantitative facts where appropriate.',
  ...
  'For Section 7 KEY RISKS, include only credible evidence-supported risks and explain why each matters. Distinguish an identifiable risk from a prediction, and do not add a generic balanced list merely to populate the section.',
  'For Section 8 OPPORTUNITIES, include only specific evidence-supported broad-market sectors, themes, or companies and explain the constructive evidence and why it matters. Keep the section broad-market rather than portfolio-limited, distinguish positive evidence from a speculative scenario, never present an opportunity as guaranteed or as a recommendation, never use generic filler such as treating a possible rebound as a buying opportunity, and never attach an unrelated reference merely to satisfy validation. If no defensible opportunity is supported, set content to null and evidenceRefs and telemetryRefs to [], use DEGRADED status, include a genuine section uncertainty, and add the corresponding material evidence gap to top-level evidenceGaps.',
  'For Section 9 WHAT TO WATCH FOR NEXT, every factual or watch-next statement must be grounded in one or more valid supplied evidenceRefs listed in Section 9. Cite each scheduled event or catalyst with its supplied supporting evidenceRef. Omit unsupported factual predictions, events, dates, earnings, macro releases, catalysts, or forward-looking developments; do not invent them or attach an unrelated reference. If the supplied package does not support a meaningful Section 9, set content to null, evidenceRefs and telemetryRefs to [], and status to DEGRADED; include at least one genuine section uncertainty and add the corresponding material evidence gap to the top-level evidenceGaps array.',
  'Review evidenceContext.subsequentDevelopments and treat those items only as later/current or forward-looking context. Never cite subsequentDevelopments as causes of the earlier primary completed-session move. When materially relevant, incorporate them using their supplied evidence references in the appropriate forward-looking Sections 7-10, especially Section 9 WHAT TO WATCH FOR NEXT, including material risks, opportunities, next-session watch items, and takeaway implications. Do not include subsequentDevelopments when they are immaterial to the report.',
  'For Section 10 MARKETBRIEF TAKEAWAY, write two to four concise sentences answering what matters most now and integrating the broader market picture, material risks or opportunities, and key developments. Do not simply repeat Section 1 and do not make an unsupported recommendation.',
  'Keep the report sections distinct: place each supported fact or conclusion where it adds the most value, refer back briefly when another section needs it, and do not repeat the same sentence or substantially identical explanation across Sections 1, 2, 3, 6, 7, 9, and 10.',
...
function missingSectionEvidenceDiagnostic(output, input) {
  ...
  for (let sectionIndex = 0; sectionIndex < REPORT_SECTION_NAMES.length - 1; sectionIndex++) {
    ...
    const deterministicEmptySection = sectionIndex === 4
...
const CONTROLLED_UNAVAILABLE_SECTIONS = Object.freeze({
  ...
  2: 'The supplied evidence did not establish a supported principal catalyst for the market move.',
  3: 'Validated broad-market company or sector evidence was unavailable.'
});
...
function normalizeEvidenceLimitedStatus(output, input) {
  ...
  if (input?.marketPackages?.every(marketPackage =>
    ...
    unavailableIndexes.set(3, CONTROLLED_UNAVAILABLE_SECTIONS[3]);
  }
...
const GENERATED_CITATION_GAPS = Object.freeze({
  ...
  sectionFourScope: 'Broad-market company and sector support could not be validated from the generated Section 4 scope.'
});
...
function normalizeDynamicReferenceViolations(output, input) {
  ...
  const causalSection = sections[2];
  if (normalizableSection(causalSection, 2) && causalSection.content !== null
    ...
    localize(2, GENERATED_CITATION_GAPS.causality);
  }

  const focusSection = sections[3];
  if (normalizableSection(focusSection, 3) && focusSection.content !== null
    ...
    const scope = sectionFourScopeViolations(focusSection, input);
    ...
    if (scope.nonFocusEvidenceCount || scope.nonBenchmarkTelemetryCount
      ...
      localize(3, GENERATED_CITATION_GAPS.sectionFourScope);
    }
  }

  const listSection = sections[4];
  if (initiatingPortfolio.length > 0 && normalizableSection(listSection, 4)
    ...
    if (invalidEvidenceCount > 0 || invalidTelemetryCount > 0) {
      ...
      localize(4, GENERATED_CITATION_GAPS.initiatingList);
    }
  }
...
async function invokeClaudeAnalysis({
  ...
  try {
    const canonicalInput = canonicalInputCopy(input);
    const evidenceLimited = normalizeEvidenceLimitedStatus(parsed, canonicalInput);
    ...
    const output = createClaudeAnalysisOutput(
