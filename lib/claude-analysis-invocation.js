...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  ...
  'For Section 3 WHAT DROVE / IS DRIVING THE MARKET, connect only supported catalysts to observed market moves and explain interactions among drivers where materially relevant. Temporal proximity alone is not causality. Preserve the completed-session and subsequent-development distinction: never present a SUBSEQUENT_DEVELOPMENT as causing an earlier completed-session move.',
...
function normalizeEvidenceLimitedStatus(output) {
  if (!output || typeof output !== 'object' || Array.isArray(output)
      || output.status !== 'NORMAL' || !Array.isArray(output.sections)
  ...
  if (unavailableIndexes.length === 0) return output;
  ...
  for (const index of unavailableIndexes) {
    const message = `The supplied evidence did not support a reliable ${REPORT_SECTION_NAMES[index]} section.`;
    if (sections[index].uncertainties.length === 0) sections[index].uncertainties = [message];
    else sections[index].uncertainties = sections[index].uncertainties.slice();
...
async function invokeClaudeAnalysis({
  ...
  try {
    const canonicalInput = canonicalInputCopy(input);
    const normalized = normalizeEvidenceLimitedStatus(parsed);
