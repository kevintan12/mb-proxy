...
function activeSessionInstruction(modelInput) {
  ...
  return 'ACTIVE_SESSION request-specific semantics: currentSessionContext contains the exact '
    ...
    + 'only as historical comparison or baseline. Section 1 must lead with the current session and use '
    ...
    + 'unresolved current-session developments and supplied upcoming catalysts. Section 8 and top-level '
    + 'Further Readings remain governed by the supplied evidenceContext.furtherReadings only; do not add '
    + 'CURRENT_SESSION articles to Further Readings in this checkpoint.';
}
