...
const PROVIDER_REPORT_TRANSPORT_KEY = 'reportJson';
...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  `Return the complete ${REPORT_HEADER} JSON using exactly the canonical eight ordered sections: ${REPORT_SECTION_NAMES.join('; ')}. Serialize that complete report JSON as the reportJson string.`,
  ...
  'In reportJson, the FURTHER READINGS payload must be exactly {"name":"FURTHER READINGS","content":null,"evidenceRefs":[],"telemetryRefs":[],"uncertainties":[]}. MarketBrief resolves and renders Further Readings separately; do not put explanatory text, URLs, provenance, or evidence references inside Section 8.',
...
const CLAUDE_ANALYSIS_PROVIDER_JSON_SCHEMA = deepFreeze({
  ...
  required: [PROVIDER_REPORT_TRANSPORT_KEY],
  properties: {
    [PROVIDER_REPORT_TRANSPORT_KEY]: {type: 'string'}
  }
});
...
function providerStructureDiagnostic(value) {
  ...
  const reportJson = isObject ? value[PROVIDER_REPORT_TRANSPORT_KEY] : null;
  return deepFreeze({
    ...
    hasReportJson: suppliedKeys.includes(PROVIDER_REPORT_TRANSPORT_KEY),
    reportJsonIsString: typeof reportJson === 'string'
  });
}
...
function parseProviderReportJson(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new TypeError('Claude structured result has invalid provider transport shape');
  }
  if (!hasExactKeySet(value, [PROVIDER_REPORT_TRANSPORT_KEY])
      || typeof value[PROVIDER_REPORT_TRANSPORT_KEY] !== 'string') {
    throw new TypeError('Claude structured result has invalid provider transport shape');
  }
  try {
    return JSON.parse(value[PROVIDER_REPORT_TRANSPORT_KEY]);
  } catch (error) {
    throw new TypeError('Claude reportJson payload was malformed');
  }
}
...
async function invokeClaudeAnalysis({
  ...
  try {
    ...
    const transportOutput = parseProviderReportJson(parsed);
