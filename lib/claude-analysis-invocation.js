...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  ...
  'For Section 4 STOCKS & SECTORS IN FOCUS, review evidenceContext.sessionAssociations and, when materially relevant, use each associated supplied evidence reference as current-session recap or context for marketContext.primaryCompletedSessionDate, even when that evidence is also a SUBSEQUENT_DEVELOPMENT. Use it only to explain broad-market leadership and laggards, sectors, notable individual movers, and closing-session breadth, rotation, or context. Never present post-close session-associated evidence as having caused the earlier completed-session move, and never treat session association as PRINCIPAL_CATALYST eligibility. Section 4 must remain broad-market and independent of My Stocks and Watchlist; membership in either list must not determine which broad-market movers Section 4 discusses. Do not require Section 4 to use every associated reference or use an association that is immaterial.',
