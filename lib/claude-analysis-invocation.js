...
const CLAUDE_ANALYSIS_SYSTEM_PROMPT = [
  `Return a direct structured ${REPORT_HEADER} transport with status, evidenceGaps, and exactly these required section slots: ${PROVIDER_SECTION_SLOT_KEYS.join(', ')}. Each slot is a four-item array [content, evidenceRefs, telemetryRefs, uncertainties] for its canonical section in this exact order: ${REPORT_SECTION_NAMES.join('; ')}. Do not embed JSON inside strings. MarketBrief reconstructs the canonical report internally.`,
  ...
  'Section slot s8 FURTHER READINGS must be exactly [null,[],[],[]]. MarketBrief resolves and renders Further Readings separately; do not put explanatory text, URLs, provenance, or evidence references inside Section 8.',
...
const CLAUDE_ANALYSIS_PROVIDER_JSON_SCHEMA = deepFreeze({
  ...
  properties: {
    ...
    ...Object.fromEntries(PROVIDER_SECTION_SLOT_KEYS.map(key => [key, {type: 'array'}]))
  }
});
...
function compactSlotValueType(value) {
...
function emitMalformedCompactSectionSlotDiagnostics(onDiagnostics, value) {
  ...
  for (const sectionSlot of PROVIDER_SECTION_SLOT_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(value, sectionSlot)) continue;
    const slot = value[sectionSlot];
    const isArray = Array.isArray(slot);
    const slotLength = isArray ? slot.length : null;
    const valueTypes = isArray ? slot.slice(0, 4).map(compactSlotValueType) : [];
    const malformed = !isArray || slotLength !== 4
      || valueTypes[0] !== 'STRING' && valueTypes[0] !== 'NULL'
      || valueTypes[1] !== 'ARRAY'
      || valueTypes[2] !== 'ARRAY'
      || valueTypes[3] !== 'ARRAY';
...
function convertDirectProviderTransport(value, input) {
  ...
  const sections = PROVIDER_SECTION_SLOT_KEYS.map((key, index) => {
    ...
    return {
      ...
      content: payload[0],
      evidenceRefs: payload[1],
      telemetryRefs: payload[2],
      uncertainties: payload[3]
    };
  });
  return {
    status: value.status,
