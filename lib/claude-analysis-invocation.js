...
const PROVIDER_SECTION_PAYLOAD_SCHEMA = deepFreeze({
  ...
  required: PROVIDER_SECTION_PAYLOAD_KEYS.slice(),
  properties: {
...
const PROVIDER_SECTION_PLACEHOLDER_SCHEMA = deepFreeze({
  type: 'object',
  additionalProperties: false
});
...
const CLAUDE_ANALYSIS_PROVIDER_JSON_SCHEMA = deepFreeze({
  ...
  properties: {
    ...
    [PROVIDER_SECTION_TRANSPORT_KEY]: {
      ...
      properties: Object.fromEntries(REPORT_SECTION_NAMES.map((name, index) => [
        name,
        index === REPORT_SECTION_NAMES.length - 1
          ? PROVIDER_SECTION_PLACEHOLDER_SCHEMA
          : PROVIDER_SECTION_PAYLOAD_SCHEMA
      ]))
    }
  }
});
