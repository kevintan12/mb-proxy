...
function activeSessionInstruction(modelInput) {
  ...
  return 'ACTIVE_SESSION request-specific semantics: currentSessionContext contains the exact '
    ...
    + 'only as historical comparison or baseline. Section 1 must begin its first sentence with the '
    + 'current state (pre-market, live trading, post-market, today, or the current session date) before '
    + 'mentioning the previous completed session. '
...
const ACTIVE_FAILED_STATUS_GAP =
  'The generated active-session report did not establish complete analytical coverage.';
...
function normalizeEvidenceLimitedStatus(output, input) {
  ...
  const activeWithCurrentEvidence = input.analysisRequest.selectedScope === 'US'
    && currentSessionEvidenceContext(input).some(entry => entry.evidenceRefs.length > 0);
  const workingOutput = failedWithSupportedSummary ? {
    ...output, status: 'DEGRADED',
    evidenceGaps: activeWithCurrentEvidence && output.evidenceGaps.length === 0
      ? [ACTIVE_FAILED_STATUS_GAP] : output.evidenceGaps
  } : output;
...
function assertActiveReferenceIntegrity(output, input) {
  if (input.analysisRequest.selectedScope !== 'US'
      || !input.marketPackages.some(item => item.market === 'US'
        && US_ACTIVE_SESSION_STATES.includes(item.marketContext.marketState))
      || currentSessionEvidenceContext(input).every(entry => entry.evidenceRefs.length === 0)) return;
  ...
  const knownTelemetry = new Set(input.marketPackages.flatMap(item =>
    item.telemetry.benchmarkSnapshots.concat(item.telemetry.stockSnapshots)
      .map(entry => entry.reference)));
  for (const section of output.sections || []) {
    for (const [references, pattern, known] of [
      [section.evidenceRefs, /^e[1-9][0-9]*$/, knownEvidence],
      [section.telemetryRefs, /^t[1-9][0-9]*$/, knownTelemetry]
    ]) {
...
function normalizeActiveTransportMetadata(output, input) {
  if (input.analysisRequest.selectedScope !== 'US'
      || !input.marketPackages.some(item => item.market === 'US'
        && US_ACTIVE_SESSION_STATES.includes(item.marketContext.marketState))
      || currentSessionEvidenceContext(input).every(entry => entry.evidenceRefs.length === 0)
...
function normalizeDynamicReferenceViolations(output, input) {
  ...
  if (normalizableSection(sections[1], 1) && sections[1].content !== null
      ...
      && sections[1].telemetryRefs.every(reference => allTelemetry.has(reference))
      && activeWithCurrentEvidence
      && hasActiveDirectMarketCausalClaim(sections[1].content)
      && !hasPriorCompletedSessionCausalClaim(sections[1].content, input)
  ...
  if (activeWithCurrentEvidence) {
    for (let index = 2; index < REPORT_SECTION_NAMES.length - 1; index++) {
      ...
      if (!normalizableSection(section, index) || section.content === null
          ...
          || !hasActiveDirectMarketCausalClaim(section.content)) continue;
...
function normalizeActiveOptionalSections(output, input) {
  if (input.analysisRequest.selectedScope !== 'US'
      || !input.marketPackages.some(item => item.market === 'US'
        && US_ACTIVE_SESSION_STATES.includes(item.marketContext.marketState))
      || currentSessionEvidenceContext(input).every(entry => entry.evidenceRefs.length === 0)
  ...
  for (let pass = 0; pass < REPORT_SECTION_NAMES.length - 1; pass++) {
    ...
    const invalidIndexes = new Set(errors.flatMap(error => {
      const match = /^sections\[([1-6])\]:/.exec(error);
    ...
    for (const index of [...invalidIndexes].sort((a, b) => a - b)) {
      const emptyList = index === 3
        && input.portfolioContext[input.analysisRequest.initiatingList].length === 0;
      ...
      sections[index] = emptyList
        ? {...sections[index], content: EMPTY_INITIATING_LIST_CONTENT[input.analysisRequest.initiatingList],
          evidenceRefs: [], telemetryRefs: [], uncertainties: []}
        : {...sections[index], content: null, evidenceRefs: [], telemetryRefs: [],
          uncertainties: [message]};
      if (!emptyList && !evidenceGaps.includes(message)) evidenceGaps.push(message);
      events.push({stage: 'claudeAnalysisSectionNormalization', sectionIndex: index,
...
async function invokeClaudeAnalysis({
  ...
  try {
    ...
    assertActiveReferenceIntegrity(canonicalizedReferences, canonicalInput);
    ...
    const citationRepair = repairSingleActiveCurrentSessionCitation(
      canonicalizedMetadata, canonicalInput
    );
    ...
    const optionalNormalization = normalizeActiveOptionalSections(initiallyResolved, canonicalInput);
