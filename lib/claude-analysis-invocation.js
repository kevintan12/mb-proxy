...
function activeSessionInstruction(modelInput) {
  ...
  return 'ACTIVE_SESSION request-specific semantics: currentSessionContext contains the exact '
    ...
    + 'only as historical comparison or baseline. Use CURRENT_SESSION evidence in whichever analytical '
...
const ACTIVE_NO_SUPPORTED_ANALYSIS_GAP =
  'No grounded analytical section survived validation for the active session.';
...
function activeSupportedSectionCount(output) {
...
function normalizeEvidenceLimitedStatus(output, input) {
  ...
  const activeIndependentSurvival = input.analysisRequest.selectedScope === 'US'
  ...
  const failedWithSupportedAnalysis = output.status === 'FAILED'
    && (activeIndependentSurvival
      ? activeSupportedSectionCount(output) > 0
...
function normalizeActiveOptionalSections(output, input) {
  ...
  if (activeSupportedSectionCount(working) === 0
    ...
    const sections = working.sections.map((section, index) => {
      const emptyInitiatingList = index === 3
        && input.portfolioContext[input.analysisRequest.initiatingList].length === 0;
      return {...section,
        content: emptyInitiatingList
          ? EMPTY_INITIATING_LIST_CONTENT[input.analysisRequest.initiatingList] : null,
        evidenceRefs: [], telemetryRefs: [], uncertainties: []};
    });
