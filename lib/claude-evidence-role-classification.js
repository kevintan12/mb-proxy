...
const CLASSIFICATION_KEYS = Object.freeze([
  'reference', 'materiality', 'roles', 'subjects', 'reason'
]);
...
const GENERIC_BROAD_MARKET_SUBJECT = /^(?:the )?(?:(?:u s|us|united states|global) )?(?:stocks?|equities|stock market|equity market|markets?|broad market|broader market|major averages|indexes|indices)$/;
...
const CLAUDE_EVIDENCE_ROLE_CLASSIFICATION_SYSTEM_PROMPT = [
  ...
  'For HIGH or MEDIUM news evidence assigned MATERIAL_EVENT when requiresBroadMarketSubjects is true, subjects must include the exact grounded names of materially significant broad-market companies or sectors explicitly supported by the title or summary, up to five; subjects: [] is allowed only when requiresBroadMarketSubjects is false or the evidence is not a HIGH or MEDIUM MATERIAL_EVENT.',
...
function canonicalSubjects(value, evidence) {
  ...
  return value.map(subject => {
    ...
    if (!normalizedName || seen.has(identity) || !searchable.includes(normalizedName)) {
      throw new TypeError('invalid evidence role classification subjects');
    }
    seen.add(identity);
...
function createClaudeEvidenceRoleClassificationOutput(output, input) {
  ...
  const classifications = output.classifications.map((classification, index) => {
    ...
    if (evidence.requiresBroadMarketSubjects
        && evidence.item.evidenceCategory === 'news'
        && ['HIGH', 'MEDIUM'].includes(classification.materiality)
        && classification.roles.includes('MATERIAL_EVENT')
        && !subjects.some(isSpecificBroadMarketSubject)) {
      throw new TypeError('material broad-market news requires specific classification subjects');
    }
    return {
