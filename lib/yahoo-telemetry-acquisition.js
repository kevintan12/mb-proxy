...
function normalizeIntradaySession(result, {expectedSessionDate, timezone, sessionContext}) {
  ...
  for (let index = 0; index < timestamps.length; index++) {
    ...
    if (dateKey(timestamp, timezone) !== expectedSessionDate
        || timestamp * 1000 < openMs || timestamp * 1000 >= closeMs) {
  ...
  for (let index = 0; index < bars.length; index++) {
    if (bars[index].timestamp * 1000 !== openMs + index * INTRADAY_INTERVAL_MS) {
      return {session: null, rejectionCategory: 'INCOMPLETE_SESSION_COVERAGE'};
    }
  }
  const finalBar = bars[bars.length - 1];
  if (finalBar.timestamp * 1000 !== closeMs - INTRADAY_INTERVAL_MS) {
  ...
  if (row?.validOhlc === true && precedingRow?.validClose !== true) {
    ...
    if (!precedingGuardReason) {
      ...
      if (!intraday || !validPositive(intraday.close)) {
        ...
        const candidate = {
          open: precedingRow?.open,
          high: precedingRow?.high,
          low: precedingRow?.low,
          close: intraday.close
        };
        let mergeRejected = precedingRow?.close !== null && precedingRow?.close !== undefined
          && !validPositive(precedingRow.close);
        for (const field of ['open', 'high', 'low']) {
          if (candidate[field] === null || candidate[field] === undefined) {
            candidate[field] = intraday[field];
          } else if (!validPositive(candidate[field])) {
            mergeRejected = true;
          }
        }
        if (mergeRejected || candidate.open !== intraday.open
            || candidate.high < intraday.high || candidate.low > intraday.low) {
          precedingRejectionCategory = 'DAILY_INTRADAY_OHLC_CONFLICT';
          precedingAttemptResult = 'FAILURE';
        } else if (!validOhlcBar(candidate.open, candidate.high, candidate.low, candidate.close)) {
          precedingRejectionCategory = 'INVALID_RECONSTRUCTED_OHLC';
          precedingAttemptResult = 'FAILURE';
        } else {
          recoveredPrecedingClose = candidate.close;
  ...
  if (!guardReason) {
    ...
    if (!intradayResult.session || !validPositive(intradayResult.session.close)) {
      ...
      for (const field of ['open', 'high', 'low']) {
        ...
        else if (!validPositive(candidate[field])) mergeRejected = true;
      }
      if (mergeRejected || candidate.open !== intraday.open || candidate.high < intraday.high
          || candidate.low > intraday.low) {
        rejectionCategory = 'DAILY_INTRADAY_OHLC_CONFLICT';
        attemptResult = 'FAILURE';
      } else if (!validOhlcBar(candidate.open, candidate.high, candidate.low, candidate.close)) {
        rejectionCategory = 'INVALID_RECONSTRUCTED_OHLC';
        attemptResult = 'FAILURE';
      } else {
