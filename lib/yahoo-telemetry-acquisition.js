...
function normalizeIntradaySession(result, {expectedSessionDate, timezone, sessionContext}) {
  ...
  if (!Array.isArray(timestamps) || timestamps.length === 0
      ...
      || ![opens, highs, lows, closes].every(values => Array.isArray(values)
        && values.length === timestamps.length)) return null;
  ...
  if (!Number.isFinite(openMs) || !Number.isFinite(closeMs)
      || closeMs <= openMs || (closeMs - openMs) % INTRADAY_INTERVAL_MS !== 0) return null;
  ...
  for (let index = 0; index < timestamps.length; index++) {
    ...
    if (!Number.isFinite(timestamp) || (previousTimestamp !== null && timestamp <= previousTimestamp)
        || dateKey(timestamp, timezone) !== expectedSessionDate
        || timestamp * 1000 < openMs || timestamp * 1000 >= closeMs) return null;
    const bar = {timestamp, open: opens[index], high: highs[index], low: lows[index], close: closes[index]};
    if (!validOhlcBar(bar.open, bar.high, bar.low, bar.close)) return null;
    bars.push(bar);
    previousTimestamp = timestamp;
  }

  const expectedCount = (closeMs - openMs) / INTRADAY_INTERVAL_MS;
  if (bars.length !== expectedCount) return null;
  for (let index = 0; index < bars.length; index++) {
    if (bars[index].timestamp * 1000 !== openMs + index * INTRADAY_INTERVAL_MS) return null;
  }
  const finalBar = bars[bars.length - 1];
  if (finalBar.timestamp * 1000 !== closeMs - INTRADAY_INTERVAL_MS) return null;
  return {
    open: bars[0].open,
...
async function applyLatestCompletedCloseFallback({
  ...
  if (!row || (row.close !== null && row.close !== undefined)) return;
  const sessionContext = getSessionContext({market, exchangeDate: expectedSessionDate});
  if (!sessionContext.calendarSupported || !sessionContext.tradingDay
      || !sessionContext.regularOpenTime || !sessionContext.regularCloseTime
      || Date.parse(sessionContext.regularCloseTime) > acquiredAt.getTime()) return;
  const intraday = await acquireIntradaySession({
    fetchImpl, symbol, expectedSessionDate, timezone, sessionContext
  });
  if (!intraday || !validPositive(intraday.close)) return;

  const candidate = {open: row.open, high: row.high, low: row.low, close: intraday.close};
  for (const field of ['open', 'high', 'low']) {
    if (candidate[field] === null || candidate[field] === undefined) candidate[field] = intraday[field];
    else if (!validPositive(candidate[field])) return;
  }
  if (candidate.open !== intraday.open || candidate.high < intraday.high || candidate.low > intraday.low
      || !validOhlcBar(candidate.open, candidate.high, candidate.low, candidate.close)) return;

  row.open = candidate.open;
  row.high = candidate.high;
  row.low = candidate.low;
  row.close = candidate.close;
  updateRowValidity(row);
}
...
function createYahooTelemetryAcquisitionService({
  ...
  return Object.freeze({
    async acquireSnapshot(input) {
      ...
      await applyLatestCompletedCloseFallback({
        market,
        rows,
        symbol,
