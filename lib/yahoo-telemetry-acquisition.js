...
function applyLatestCompletedCloseFallback({
  market,
  rows,
  expectedSessionDate,
  meta,
  ...
  if (!validPositive(price) || !Number.isFinite(timestamp)
      || dateKey(timestamp, timezone) !== expectedSessionDate
      || timestamp * 1000 > acquiredAt.getTime()) return;
  ...
  if (!sessionContext.calendarSupported || !sessionContext.tradingDay
      || !sessionContext.regularCloseTime
      || timestamp * 1000 < Date.parse(sessionContext.regularCloseTime)
      || !validPositive(row.low) || !validPositive(row.high)
      || price < row.low || price > row.high) return;
...
function createYahooTelemetryAcquisitionService({
  ...
  return Object.freeze({
    async acquireSnapshot(input) {
      ...
      applyLatestCompletedCloseFallback({
        market,
        rows,
