...
const TOOL_FAILURE_TYPES = new Set([
  'invalid_input',
  'max_uses_exceeded',
  'query_too_long',
  'too_many_requests',
...
function searchResults(envelope) {
  if (!envelope || !Array.isArray(envelope.content)) {
    return {valid: false, error: false, toolFailureType: null, results: []};
  }
  let valid = false;
  let error = false;
  let toolFailureType = null;
  const results = [];
  for (const block of envelope.content) {
    ...
    for (const item of (Array.isArray(block.content) ? block.content : [block.content])) {
      if (item?.type === 'web_search_tool_result_error') {
        error = true;
        if (toolFailureType === null && TOOL_FAILURE_TYPES.has(item.error_code)) {
          toolFailureType = item.error_code;
        }
      }
      else if (item?.type === 'web_search_result') results.push(item);
    }
  }
  return {valid, error, toolFailureType, results};
}
...
function providerRequestId(upstream) {
  try {
    const value = upstream?.headers?.get('request-id');
...
function providerUsage(usage) {
  ...
  ].flatMap(key => typeof usage[key] === 'number' && Number.isFinite(usage[key])
    ? [[key, usage[key]]] : []));
}

function providerSearchRequestCount(usage) {
...
function createClaudeBoundedCnbcMarketNewsDiscoveryService({
  apiKey,
  ...
  const cache = cnbcMarketNewsDiscoveryCache
    && typeof cnbcMarketNewsDiscoveryCache.get === 'function'
    && typeof cnbcMarketNewsDiscoveryCache.set === 'function'
    && typeof cnbcMarketNewsDiscoveryCache.delete === 'function'
    ? cnbcMarketNewsDiscoveryCache : null;
  const resultMetadata = new WeakMap();

  function emitCache(searchIndex, outcome) {
    emit(onDiagnostics, {
      stage: 'cnbcMarketNewsDiscoveryCache',
      provider: 'CNBC',
      searchIndex,
      outcome
    });
  ...
  function metadataFor(result) {
    return result && typeof result === 'object' ? resultMetadata.get(result) || null : null;
  }
  ...
  const service = {
    ...
    evictCachedSearchIndexes({targetSessionDate, searchIndexes} = {}) {
      if (!cache || !Array.isArray(searchIndexes)) return false;
      let deleted = false;
      for (const searchIndex of [...new Set(searchIndexes)].sort()) {
        if (cache.delete({provider: 'CNBC', targetSessionDate, searchIndex})) {
          deleted = true;
          emitCache(searchIndex, 'EVICTED_AFTER_VALIDATION_FAILURE');
        }
      }
      return deleted;
    },
    completeValidatedDiscoveries({targetSessionDate, discoveries: validated, cachedSearchIndexes} = {}) {
      if (!cache || !Array.isArray(validated)) return false;
      const bySearch = new Map(SEARCH_INTENTS.map((_, index) => [index + 1, []]));
      for (const discovery of validated) {
        const searchIndex = Math.floor((discovery?.rank - 1) / MAX_RESULTS_INSPECTED_PER_SEARCH) + 1;
        if (!bySearch.has(searchIndex)) continue;
        bySearch.get(searchIndex).push({
      ...
      let stored = false;
      const cached = new Set(Array.isArray(cachedSearchIndexes) ? cachedSearchIndexes : []);
      for (const [searchIndex, identities] of bySearch) {
        if (!identities.length) continue;
        if (cache.set({provider: 'CNBC', targetSessionDate, searchIndex, identities})) {
          stored = true;
          if (cached.has(searchIndex)) emitCache(searchIndex, 'VALIDATED_HIT');
        }
      }
      return stored;
    },
    async discoverCnbcMarketNews(input) {
      ...
      const cachedSearchIndexes = [];
      let firstFailure = null;
      let rawResultCount = 0;
      for (const [searchIndex, body] of bodies.entries()) {
        const oneBasedSearchIndex = searchIndex + 1;
        const cached = cache?.get({
          provider: 'CNBC', targetSessionDate: input.targetSessionDate,
          searchIndex: oneBasedSearchIndex
        });
        if (cached) {
          cachedSearchIndexes.push(oneBasedSearchIndex);
          inspectedResultSets[searchIndex] = cached.map(identity => ({
        ...
        if (cache) {
          emitCache(oneBasedSearchIndex, 'MISS');
          emitCache(oneBasedSearchIndex, 'FALLBACK_DISCOVERY');
        }
        ...
        const emitInvocation = (outcome, toolFailureType = null) => emit(onDiagnostics, {
          stage: 'cnbcMarketNewsSearchInvocation', model: MODEL, searchIndex: searchIndex + 1,
          outcome, requestId: providerRequestId(upstream),
          requestSize: {
            systemPromptBytes: Buffer.byteLength(requests[searchIndex].system, 'utf8'),
            userPromptBytes: Buffer.byteLength(requests[searchIndex].messages[0].content, 'utf8'),
            completeRequestBodyBytes: requestBytes[searchIndex]
          },
          elapsedMs: Math.max(0, monotonicNow() - invocationStarted),
          usage: providerUsage(envelope?.usage),
          searchRequestCount: providerSearchRequestCount(envelope?.usage),
          httpStatus: Number.isInteger(upstream?.status) ? upstream.status : null,
          ...(upstreamError || {}),
          ...(outcome === 'SEARCH_TOOL_FAILURE' ? {
            toolFailureType: toolFailureType || 'UNKNOWN_TOOL_FAILURE'
          } : {})
        });
        try {
          ...
          emitInvocation('NETWORK_FAILURE');
          continue;
        }
        if (!upstream?.ok) {
          upstreamError = await readAnthropicErrorDiagnostics(upstream);
          ...
          emitInvocation('UPSTREAM_FAILURE');
          continue;
        }
        try { envelope = await upstream.json(); } catch (error) {
          ...
          emitInvocation('RESPONSE_READ_FAILURE');
          continue;
        }
        ...
        if (!parsed.valid) {
          ...
          emitInvocation('INVALID_SEARCH_RESULT');
          continue;
        }
        if (parsed.error) {
          firstFailure ||= failure('SEARCH_TOOL_FAILURE', 'Claude web search tool failed', upstream.status ?? null);
          emitInvocation('SEARCH_TOOL_FAILURE', parsed.toolFailureType);
          continue;
        }
      ...
      resultMetadata.set(result, deepFreeze({
        cachedSearchIndexes: cachedSearchIndexes.slice()
      }));
      return result;
    }
  };
