...
function discoveries(results, targetSessionDate) {
  ...
  for (const [index, result] of results.entries()) {
    ...
    if (output.length === MAX_DISCOVERIES) {
      rejectionCounts.RETAINED_LIMIT++;
      continue;
    }
  ...
  return Object.freeze({
    async discoverCnbcMarketNews(input) {
      ...
      for (const body of bodies) {
        ...
        inspectedResults.push(...parsed.results.slice(0, MAX_RESULTS_INSPECTED_PER_SEARCH));
      ...
      const selected = discoveries(inspectedResults, input.targetSessionDate);
      const outcome = selected.output.length ? (firstFailure ? 'PARTIAL_SUCCESS' : 'SUCCESS')
        : rawResultCount === 0 && !firstFailure ? 'ZERO_RESULTS'
          : inspectedResults.length > 0 && !firstFailure ? 'ALL_RESULTS_REJECTED'
      ...
      emit(onDiagnostics, {
        ...
        inspectedResultCount: inspectedResults.length,
