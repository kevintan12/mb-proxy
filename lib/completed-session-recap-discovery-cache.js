...
function canonicalDiscovery(discovery, targetSessionDate, provider) {
  if (!hasExactKeys(discovery, DISCOVERY_KEYS)
      ...
      || (discovery.discoveredVia !== 'ANTHROPIC_WEB_SEARCH'
        && !(provider === 'CNBC' && discovery.discoveredVia === 'DETERMINISTIC_SESSION_URL'))) return null;
  return deepFreeze({
...
function createCompletedSessionRecapDiscoveryCache() {
  ...
  return Object.freeze({
    ...
    set({provider, targetSessionDate, discovery} = {}) {
      const canonical = canonicalProvider(provider);
      const date = canonicalDate(targetSessionDate);
      const identity = date ? canonicalDiscovery(discovery, date, canonical) : null;
