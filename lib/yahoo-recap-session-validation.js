...
function extractMetadata(html, expectedUrl, targetSessionDate, maxHeadlineBytes) {
  ...
  if (canonical !== null && canonicalYahooRecapUrl(canonical) !== expectedUrl) {
    return {metadata: null, timestampOrderRejected: false};
  }
  const scripts = [];
  ...
  let reversedModificationFallback = null;
  for (const article of nodes.filter(isArticle)) {
    ...
    if (dateModified !== null && Date.parse(dateModified) < Date.parse(datePublished)) {
      reversedModificationFallback ??= {headline, datePublished, dateModified: null};
      continue;
    }
    return {metadata: {headline, datePublished, dateModified}, timestampOrderRejected: false};
  }
  return {metadata: reversedModificationFallback, timestampOrderRejected: false};
}
  ...
  return deepFreeze({
    async validateYahooRecapSession(input) {
      ...
      if (!extracted.metadata) {
        finish(extracted.timestampOrderRejected
          ? 'NOT_VALIDATED_TIMESTAMP_ORDER' : 'NOT_VALIDATED');
