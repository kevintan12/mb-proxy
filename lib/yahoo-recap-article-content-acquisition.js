...
function extractArticle(html, canonical) {
  ...
  if (compatibleArticles.length === 0) {
    ...
    const diagnosticFailureType = canonicalUrlMismatch ? 'CANONICAL_URL_MISMATCH'
      : dateMismatch ? 'DATE_MISMATCH'
        : invalidPublishedAt ? 'INVALID_DATE_PUBLISHED' : failureType;
  ...
  if (!selected) {
    return invalidBodyMetadata
      ? {failureType: 'INVALID_METADATA', diagnosticFailureType: invalidDateModified
          ? 'INVALID_DATE_MODIFIED' : 'INVALID_METADATA'}
  ...
  if (updatedAtValues.size > 1 || publisherNames.size > 1) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'INVALID_METADATA'};
  }
  const updatedAt = updatedAtValues.size === 0 ? null : [...updatedAtValues][0];
  if (updatedAt !== canonical.validation.dateModified || publisherNames.size === 0) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'INVALID_METADATA'};
  }
  return {
  ...
  return deepFreeze({
    async acquireArticleContent(input) {
      ...
      if (Buffer.byteLength(extracted.headline, 'utf8') > canonical.bounds.maxHeadlineBytes) {
        return finish(result(false, 'INVALID_METADATA', null,
