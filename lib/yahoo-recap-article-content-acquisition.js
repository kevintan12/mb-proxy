...
function extractArticle(html, canonical) {
  ...
  for (const article of articles) {
    ...
    if (!publishedAt || publishedAt !== canonical.validation.datePublished
        || newYorkDate(publishedAt) !== canonical.discovery.targetSessionDate) continue;
  ...
  if (!selected) {
    return {failureType: invalidBodyMetadata ? 'INVALID_METADATA' : 'ARTICLE_BODY_MISSING'};
  }
  ...
  if (updatedAtValues.size > 1 || publisherNames.size > 1) {
    return {failureType: 'INVALID_METADATA'};
  }
