...
const MAX_LIVE_BLOG_UPDATES_INSPECTED = 50;
const MAX_CPI_COMPARATIVE_EXCERPTS = 8;
...
async function readBoundedBody(response, signal, maxBytes) {
  ...
  if (Number.isFinite(declared) && declared > maxBytes) {
    return {tooLarge: true, html: null, bytes: declared};
  }
  ...
  return {tooLarge, html: tooLarge ? null : html, bytes};
}
...
function canonicalPublisher(publisher, maxPublisherNameBytes) {
  ...
  if (!name || name !== publisher.name
      || Buffer.byteLength(name, 'utf8') > maxPublisherNameBytes) return null;
  return deepFreeze({name});
}
...
function jsonLdNodes(value, output = []) {
  ...
  Object.entries(value).forEach(([key, item]) => {
    if (key !== 'liveBlogUpdate' && item && typeof item === 'object') {
      jsonLdNodes(item, output);
    }
  });
  return output;
}
...
function cpiComparativeExcerpts(article) {
  if (!hasType(article, 'LiveBlogPosting') || !Array.isArray(article.liveBlogUpdate)) return [];
  const excerpts = [];
  const seen = new Set();
  for (const update of article.liveBlogUpdate.slice(0, MAX_LIVE_BLOG_UPDATES_INSPECTED)) {
    if (!hasType(update, 'BlogPosting')) continue;
    const text = normalizePlainText(update.articleBody);
    if (!text) continue;
    for (const clause of text.split(/(?<=[.!?])\s+|;\s*/)) {
      const value = clause.trim();
      if (!value || !/(?:\bCPI\b|consumer price index|consumer inflation|["“]?core["”]? inflation)/i.test(value)
          || !/(?:month(?:-on-| over )month|monthly basis|year(?:-on-| over )year|annual basis)/i.test(value)
          || !/[+-]?\d+(?:\.\d+)?%/.test(value)
          || !/(?:expect(?:ed|ation|ations)?|estimate(?:s|d)?|previous(?:ly)?|unchanged|faster|slower|hotter|cooler)/i.test(value)
          || seen.has(value)) continue;
      seen.add(value);
      excerpts.push(value);
      if (excerpts.length === MAX_CPI_COMPARATIVE_EXCERPTS) return excerpts;
    }
  }
  return excerpts;
}
...
function extractArticle(html, canonical) {
  ...
  if (pageCanonical !== null && canonicalYahooRecapUrl(pageCanonical) !== canonical.canonicalUrl) {
    return {failureType: 'IDENTITY_MISMATCH', diagnosticFailureType: 'CANONICAL_URL_MISMATCH'};
  }
  const scripts = [];
  ...
  if (scripts.length === 0) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'INVALID_JSON_LD'};
  }
  const nodes = [];
  ...
  let canonicalUrlMismatch = false;
  let invalidPublishedAt = false;
  let dateMismatch = false;
  let missingMetadataUrl = false;
  let missingHeadline = false;
  ...
  for (const article of articles) {
    ...
    if (!hasMetadataUrl) {
      missingMetadataUrl = true;
      continue;
    }
    const articleUrl = metadataUrl(article);
    if (!articleUrl || articleUrl !== canonical.canonicalUrl) {
      identityMismatch = true;
      canonicalUrlMismatch = true;
      continue;
    }
    const publishedAt = canonicalTimestamp(article.datePublished);
    if (!publishedAt) {
      invalidPublishedAt = true;
      continue;
    }
    if (publishedAt !== canonical.validation.datePublished
        || newYorkDate(publishedAt) !== canonical.discovery.targetSessionDate) {
      dateMismatch = true;
      continue;
    }
    ...
    compatibleArticles.push({article, headline, publishedAt});
  }
  if (compatibleArticles.length === 0) {
    const failureType = identityMismatch ? 'IDENTITY_MISMATCH' : 'INVALID_METADATA';
    const diagnosticFailureType = canonicalUrlMismatch ? 'CANONICAL_URL_MISMATCH'
      : dateMismatch ? 'DATE_MISMATCH'
        : invalidPublishedAt ? 'INVALID_DATE_PUBLISHED'
          : missingMetadataUrl ? 'MISSING_METADATA_URL'
            : missingHeadline ? 'MISSING_HEADLINE' : failureType;
    return {failureType, diagnosticFailureType};
  }

  let selected = null;
  let invalidBodyMetadata = false;
  let invalidDateModified = false;
  let invalidPublisher = false;
  for (const compatible of compatibleArticles) {
    const articleText = normalizePlainText(compatible.article.articleBody);
    if (!articleText) continue;
    const hasPublisher = compatible.article.publisher !== undefined
      && compatible.article.publisher !== null;
    const publisher = hasPublisher
      ? canonicalPublisher(compatible.article.publisher, canonical.bounds.maxPublisherNameBytes)
      : null;
    const hasUpdatedAt = compatible.article.dateModified !== undefined
      && compatible.article.dateModified !== null;
    const updatedAt = hasUpdatedAt ? canonicalTimestamp(compatible.article.dateModified) : null;
    if ((hasPublisher && !publisher) || (hasUpdatedAt && !updatedAt)) {
      invalidBodyMetadata = true;
      if (hasPublisher && !publisher) invalidPublisher = true;
      if (hasUpdatedAt && !updatedAt) invalidDateModified = true;
      continue;
    }
    const cpiExcerpts = cpiComparativeExcerpts(compatible.article);
    selected = {
      ...compatible,
      articleText: cpiExcerpts.length === 0
        ? articleText
        : `${articleText} ${cpiExcerpts.join('; ')}`
    };
    break;
  }
  if (!selected) {
    return invalidBodyMetadata
      ? {failureType: 'INVALID_METADATA', diagnosticFailureType: invalidDateModified
          ? 'INVALID_DATE_MODIFIED_FORMAT'
          : invalidPublisher ? 'INVALID_PUBLISHER' : 'INVALID_METADATA'}
      : {failureType: 'ARTICLE_BODY_MISSING', diagnosticFailureType: 'ARTICLE_BODY_MISSING'};
  }

  const metadataArticles = compatibleArticles.filter(
    compatible => compatible.headline === selected.headline
  );
  const updatedAtValues = new Set();
  const publisherNames = new Set();
  let invalidMetadataPublisher = false;
  for (const {article} of metadataArticles) {
    if (article.dateModified !== undefined && article.dateModified !== null) {
      const updatedAt = canonicalTimestamp(article.dateModified);
      if (updatedAt) updatedAtValues.add(updatedAt);
    }
    if (article.publisher !== undefined && article.publisher !== null) {
      const publisher = canonicalPublisher(
        article.publisher,
        canonical.bounds.maxPublisherNameBytes
      );
      if (publisher) publisherNames.add(publisher.name);
      else invalidMetadataPublisher = true;
    }
  }
  if (updatedAtValues.size > 1) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'DATE_MODIFIED_CONFLICT'};
  }
  if (publisherNames.size > 1) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'PUBLISHER_CONFLICT'};
  }
  let updatedAt = updatedAtValues.size === 0 ? null : [...updatedAtValues][0];
  if (updatedAt !== null && Date.parse(updatedAt) < Date.parse(selected.publishedAt)
      && canonical.validation.dateModified !== null) {
    return {
      failureType: 'INVALID_METADATA',
      diagnosticFailureType: 'DATE_MODIFIED_BEFORE_PUBLISHED'
    };
  }
  if (updatedAt !== null && Date.parse(updatedAt) < Date.parse(selected.publishedAt)) {
    updatedAt = null;
  }
  if (canonical.validation.dateModified !== null
      && (updatedAt === null
        || Date.parse(updatedAt) < Date.parse(canonical.validation.dateModified))) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: 'DATE_MODIFIED_MISMATCH'};
  }
  if (publisherNames.size === 0) {
    return {failureType: 'INVALID_METADATA', diagnosticFailureType: invalidMetadataPublisher
        ? 'INVALID_PUBLISHER' : 'PUBLISHER_MISSING'};
  }
  return {
    headline: selected.headline,
    publisher: deepFreeze({name: [...publisherNames][0]}),
    publishedAt: selected.publishedAt,
    updatedAt,
    articleText: selected.articleText
  };
}
...
function createYahooRecapArticleContentAcquisitionService({
  ...
  return deepFreeze({
    async acquireArticleContent(input) {
      ...
      const finish = (value, failureType = value.ok === true ? null : value.type) => {
        emitDiagnostics(onDiagnostics, {
          stage: 'yahooRecapArticleAcquisition',
          outcome: value.ok === true ? 'SUCCESS' : 'FAILURE',
          httpStatus,
          responseBytes,
          contentType,
          elapsedMs: elapsedMilliseconds(started, monotonicNow()),
          failureType,
          candidateRank: null
        });
        return value;
      ...
      if (!canonical) {
        return finish(result(false, 'INVALID_INPUT', null, 'Invalid Yahoo recap article request'));
      }
      ...
      try {
        try {
          ...
          return finish(result(false, type, null, 'Yahoo recap article could not be retrieved'));
        }
        httpStatus = Number.isInteger(response?.status) ? response.status : null;
        if (!response?.ok) {
          return finish(result(false, 'HTTP_FAILURE', null, 'Yahoo recap article request was unsuccessful'));
        }
        const responseUrl = response.url
          ? canonicalYahooRecapUrl(response.url) : canonical.canonicalUrl;
        if (!responseUrl || responseUrl !== canonical.canonicalUrl) {
          return finish(result(false, 'IDENTITY_MISMATCH', null,
            'Yahoo recap article identity does not match'), 'CANONICAL_URL_MISMATCH');
        }
        contentType = responseContentType(response) ? 'HTML' : 'UNSUPPORTED';
        if (contentType !== 'HTML') {
          return finish(result(false, 'INVALID_CONTENT_TYPE', null,
            'Yahoo recap article response is not HTML'));
        }
        try {
          body = await readBoundedBody(response, controller.signal, canonical.bounds.maxResponseBytes);
        } catch (error) {
          const type = isAbort(error, controller.signal) ? 'TIMEOUT' : 'RETRIEVAL_FAILURE';
          return finish(result(false, type, null, 'Yahoo recap article response could not be read'));
        }
      } finally {
      ...
      if (body.tooLarge) {
        return finish(result(false, 'RESPONSE_TOO_LARGE', null,
          'Yahoo recap article response exceeds configured bounds'));
      }
      ...
      if (extracted.failureType) {
        ...
        return finish(
          result(false, extracted.failureType, null, messages[extracted.failureType]),
          extracted.diagnosticFailureType || extracted.failureType
        );
      }
      if (Buffer.byteLength(extracted.headline, 'utf8') > canonical.bounds.maxHeadlineBytes) {
        return finish(
          result(false, 'INVALID_METADATA', null,
            'Yahoo recap article structured metadata is invalid'),
          'HEADLINE_TOO_LARGE'
        );
      }
      if (Buffer.byteLength(extracted.articleText, 'utf8') > canonical.bounds.maxArticleTextBytes) {
        return finish(result(false, 'ARTICLE_TEXT_TOO_LARGE', null,
          'Yahoo recap article text exceeds configured bounds'));
      }
      ...
      if (!hasExactKeys(articleContent, YAHOO_RECAP_ARTICLE_CONTENT_KEYS)
        ...
        return finish(result(false, 'RESULT_TOO_LARGE', null,
          'Yahoo recap article result exceeds configured bounds'));
      }
      return finish(result(true, 'SUCCESS', articleContent));
    }
  });
}
