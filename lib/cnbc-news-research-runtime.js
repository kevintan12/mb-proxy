...
function createCnbcNewsResearchRuntime({
  ...
  const discovery = createClaudeBoundedCnbcMarketNewsDiscoveryService({
    apiKey, fetchImpl, onDiagnostics, cnbcMarketNewsDiscoveryCache
  });
  const articleContentAcquisition = createCnbcArticleContentAcquisitionService({
    fetchImpl, onDiagnostics
  });
