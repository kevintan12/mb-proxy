...
const COMPLETED_SESSION_PROVENANCE_KEYS = Object.freeze([
  'publisher',
  'authority',
  'homepage',
  'applicableMarket',
...
function validateCompletedSessionTelemetry(record) {
  const errors = [];
  if (!record || typeof record !== 'object' || Array.isArray(record)) {
    return deepFreeze({ valid: false, errors: ['completed-session telemetry must be an object'] });
  }
  if (!hasExactKeys(record, COMPLETED_SESSION_TELEMETRY_KEYS)) {
    errors.push('invalid canonical property shape or order');
  }

  let expected = null;
  try {
    expected = createCompletedSessionTelemetry({
  ...
  if (!hasExactKeys(record.provenance, COMPLETED_SESSION_PROVENANCE_KEYS)) {
    errors.push('invalid canonical provenance');
  }

  if (expected) {
    for (const key of COMPLETED_SESSION_TELEMETRY_KEYS) {
      const equal = key === 'provenance'
        ? hasExactKeys(record.provenance, COMPLETED_SESSION_PROVENANCE_KEYS)
          && COMPLETED_SESSION_PROVENANCE_KEYS.every(provenanceKey =>
            record.provenance[provenanceKey] === expected.provenance[provenanceKey])
