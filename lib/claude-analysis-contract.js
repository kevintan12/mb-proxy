  ...
  if (!Array.isArray(output.sections) || output.sections.length !== REPORT_SECTION_NAMES.length) {
    ...
    if (activeWithCurrentEvidence && output.status !== 'FAILED') {
      ...
      if (!Array.isArray(summary?.evidenceRefs)
          || !summary.evidenceRefs.some(reference => currentSessionReferences.has(reference))) {
    ...
    if (activeWithCurrentEvidence && output.status === 'FAILED') {
      errors.push('active analysis with CURRENT_SESSION evidence cannot use FAILED status');
    }
    for (let index = 0; index < output.sections.length; index++) {
      ...
      if (activeWithCurrentEvidence && (index === 4 || index === 6) && section.content !== null
          && validEvidenceRefs
          && !section.evidenceRefs.some(reference => currentSessionReferences.has(reference))) {
        errors.push(`sections[${index}]: active section requires CURRENT_SESSION evidence first`);
      }
      if (index === 1 && section.content !== null && validEvidenceRefs
      ...
      if (activeWithCurrentEvidence && index !== 1 && index < REPORT_SECTION_NAMES.length - 1
