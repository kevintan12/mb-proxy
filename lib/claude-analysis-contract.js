...
function sectionFourScopeViolations(section, input) {
  ...
  if (!Array.isArray(output.sections) || output.sections.length !== REPORT_SECTION_NAMES.length) {
    ...
    for (let index = 0; index < output.sections.length; index++) {
      ...
      if (index === 2 && section.content !== null && validEvidenceRefs
        ...
        errors.push('sections[2]: market causality requires a principal catalyst');
      }
      if (index === 3 && section.content !== null && validEvidenceRefs) {
        const citedFocus = broadMarketFocus.filter(entry =>
          section.evidenceRefs.includes(entry.evidenceRef));
        if (citedFocus.length === 0) {
          errors.push('sections[3]: stocks and sectors require broad-market focus evidence');
        } else {
          ...
          if (!mentionsSubject) {
            errors.push('sections[3]: stocks and sectors must mention a validated broad-market subject');
          }
        }
      }
      if (index === 3 && validEvidenceRefs && validTelemetryRefs) {
        const scope = sectionFourScopeViolations(section, input);
        if (scope.nonFocusEvidenceCount) {
          errors.push('sections[3]: evidence references must belong to broad-market focus');
        }
        if (scope.nonBenchmarkTelemetryCount) {
          errors.push('sections[3]: telemetry references must belong to benchmark telemetry');
        }
        if (scope.unfocusedPortfolioMentionCount) {
          errors.push('sections[3]: portfolio company requires independent broad-market focus');
        }
      }
