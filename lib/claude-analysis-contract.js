...
const {canonicalYahooCurrentNewsArticleUrl} = require('./yahoo-current-news-article-content-acquisition');
const {
...
const PLAIN_ENGLISH_REPLACEMENTS = Object.freeze([
  Object.freeze({pattern: '\\brisk[- ]taking appetite\\b', replacement:
    "investors' willingness to take risks"}),
  Object.freeze({pattern: '\\bhigh[- ]beta names\\b', replacement:
    'stocks that often move more sharply than the broader market'}),
  Object.freeze({pattern: '\\bcyclical participants\\b', replacement:
    'companies that tend to benefit when the economy is growing'}),
  Object.freeze({pattern: '\\brisk appetite\\b', replacement:
    "investors' willingness to take risk"}),
  Object.freeze({pattern: '\\basymmetric risk[- ]reward\\b', replacement:
    'uneven balance between possible gains and losses'}),
  Object.freeze({pattern: '\\bconsolidation thesis\\b', replacement:
    'case for the market to pause after recent moves'}),
  Object.freeze({pattern: '\\bselective sentiment\\s*\\/\\s*positioning\\b', replacement:
    'investors were positive about only some areas and had already committed money selectively'}),
  Object.freeze({pattern: '\\bselective sentiment\\b', replacement:
    'investors were positive about only some areas'}),
  Object.freeze({pattern: '\\bgrowth[- ]oriented stocks\\b', replacement:
    'shares of companies expected to grow quickly'}),
  Object.freeze({pattern: '\\briskier smaller[- ]capitalization holdings\\b', replacement:
    'riskier shares of smaller companies'}),
  Object.freeze({pattern: '\\bmarket is repricing downward\\b', replacement:
    'market is falling'}),
  Object.freeze({pattern: '\\brate-path expectations\\b', replacement:
    'expectations for future interest rates'}),
  Object.freeze({pattern: '\\bsector rotation\\b', replacement:
    'money moving between sectors'}),
  Object.freeze({pattern: '\\breallocation momentum\\b', replacement:
    'investors shifting money between investments'}),
  Object.freeze({pattern: '\\brate-sensitive sectors\\b', replacement:
    'sectors especially affected by interest-rate changes'})
]);
// Context-dependent jargon cannot be rewritten without risking a changed claim. It is
// style-only (D-001): detected and counted for diagnostics, never section-fatal.
const PLAIN_ENGLISH_STYLE_PATTERNS = Object.freeze([
  /\b(?:hawkish|dovish)\b/i,
  /\bmonetary restraint\b/i,
  /\bflight to quality\b/i,
  /\b(?:risk|market) (?:exposure|appetite)\b/i,
  /\b(?:high|low)[- ](?:multiple|beta|conviction)(?:[- ]valuations?)?\b/i,
  /\b(?:small|mid|large|smaller)[- ]capitalization\b/i,
  /\b(?:growth|value)[- ]oriented\b/i,
  /\b(?:reprice|reprices|repriced|repricing)(?: downward)?\b/i,
  /\bpositioning\b/i,
  /\bhow investors are already invested\b/i
]);
// Closed list of broken replacement prose (a rewritten clause spliced into a subject
// slot). Only these signatures, plus structurally invalid content, may localize a section.
const MALFORMED_PLAIN_ENGLISH_PATTERNS = Object.freeze([
  /\b(?:healthcare|technology|financials|energy|sector|stocks|shares)\s+how investors are already invested\b/i
]);
...
function normalizeMarketPackage(input, expectedMarket, nextEvidenceReference, nextTelemetryReference) {
  ...
  if (market === 'US' && US_ACTIVE_SESSION_STATES.includes(marketState)) {
    const anchor = parseUsActiveSessionAnchor(calendarContext);
    if (!anchor || anchor.marketState !== marketState
        || !activeSessionEvidenceWindowResult({
          marketState,
          benchmarkSnapshots,
          anchor
        }).window) throw new TypeError('invalid active-session calendar context');
  }
...
function validateClaudeAnalysisInput(input) {
  ...
  for (let packageIndex = 0; packageIndex < input.marketPackages.length; packageIndex++) {
    ...
    if (marketPackage.market === 'US' && US_ACTIVE_SESSION_STATES.includes(context.marketState)) {
      const anchor = parseUsActiveSessionAnchor(context.calendarContext);
      if (!anchor || anchor.marketState !== context.marketState
          || !activeSessionEvidenceWindowResult({
...
function hasCanonicalYahooFinanceUrl(value) {
  return canonicalYahooCurrentNewsArticleUrl(value) !== null;
}

function normalizePlainEnglishText(value) {
  if (typeof value !== 'string') return value;
  return PLAIN_ENGLISH_REPLACEMENTS.reduce((text, {pattern, replacement}) => text.replace(
    new RegExp(pattern, 'gi'), match => /^[A-Z]/.test(match)
      ? `${replacement.slice(0, 1).toUpperCase()}${replacement.slice(1)}` : replacement
  ), value);
}

function hasAnalystDeskJargon(value) {
  if (typeof value !== 'string') return false;
  return PLAIN_ENGLISH_REPLACEMENTS.some(({pattern}) => new RegExp(pattern, 'i').test(value))
    || PLAIN_ENGLISH_STYLE_PATTERNS.some(pattern => pattern.test(value));
}

function countAnalystDeskJargon(value) {
  if (typeof value !== 'string') return 0;
  return PLAIN_ENGLISH_REPLACEMENTS.map(({pattern}) => new RegExp(pattern, 'gi'))
    .concat(PLAIN_ENGLISH_STYLE_PATTERNS.map(pattern => new RegExp(pattern.source, 'gi')))
    .reduce((count, pattern) => count + (value.match(pattern) || []).length, 0);
}

function hasMalformedPlainEnglishProse(value) {
  if (typeof value !== 'string') return false;
  return MALFORMED_PLAIN_ENGLISH_PATTERNS.some(pattern => pattern.test(value));
}
...
function hasActiveDirectMarketCausalClaim(content) {
  if (typeof content !== 'string') return false;
  // A session day/date following "on" identifies when a move occurred, not what caused it.
  const withoutTemporalOn = content.replace(
    /\bon\s+(?:(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b|(?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\.?\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?\b|\d{4}-\d{2}-\d{2}\b|\d{1,2}\/\d{1,2}\/\d{2,4}\b)/gi,
    'during the session'
  );
  return hasDirectMarketCausalClaim(withoutTemporalOn);
}

function hasPriorCompletedSessionFirstFraming(content, input) {
  if (typeof content !== 'string') return false;
  const firstSentence = content.split(/[.!?](?:\s|$)/, 1)[0].trim().replace(
    /\b(Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.(?=\s+\d{1,2}\b)/gi,
    '$1'
  );
  const priorOpening = /^(?:at|by|during|in|on|since)?\s*(?:the\s+)?(?:(?:previous|prior|earlier|last)\s+(?:(?:trading\s+)?session|(?:session\s+)?close)|completed(?:-|\s+)(?:trading(?:-|\s+))?session(?:\s+close)?|yesterday(?:['’]s)?|(?:monday|tuesday|wednesday|thursday|friday)(?:['’]s)?\s+close)\b/i;
  const completedDate = input.marketPackages.find(item => item.market === 'US')
    ?.marketContext?.primaryCompletedSessionDate;
  const completedWeekday = /^\d{4}-\d{2}-\d{2}$/.test(completedDate || '')
    ? new Intl.DateTimeFormat('en-US', {weekday: 'long', timeZone: 'UTC'})
      .format(new Date(`${completedDate}T12:00:00.000Z`))
    : null;
  const weekdayOpening = completedWeekday
    ? new RegExp(`^(?:on\\s+)?${completedWeekday}(?:['’]s)?\\b`, 'i') : null;
  return priorOpening.test(firstSentence)
    || Boolean(weekdayOpening?.test(firstSentence))
    || completedSessionDatePatterns(input).some(pattern => {
      const match = pattern.exec(firstSentence);
      return match && match.index <= 12;
    });
}

function activeSessionOpeningDatePatterns(input) {
  const dates = [...new Set(currentSessionEvidenceContext(input)
  ...
  return dates.flatMap(value => {
    ...
    return [
      ...
      new RegExp(`\\b(?:${longMonth}|${shortMonth}\\.?)\\s+0?${day}(?:st|nd|rd|th)?(?:,?\\s+${year})?\\b`, 'i'),
      new RegExp(`\\b0?${month}/0?${day}/${year}\\b`)
    ];
  });
}

function hasActiveSessionOpening(content, input) {
  if (typeof content !== 'string') return false;
  const state = input.marketPackages.find(item => item.market === 'US')?.marketContext?.marketState;
  const firstSentence = content.split(/[.!?](?:\s|$)/, 1)[0].trim();
  if (!firstSentence || hasPriorCompletedSessionFirstFraming(firstSentence, input)) return false;
  const currentMarker = /\b(?:current(?:[-\s]session| market)?|today|now|live|this\s+(?:morning|afternoon|evening|session)|midday|opening|opened)\b/i;
  const stateMarker = state === 'PRE'
    ? /\b(?:pre[-\s]?market|before\s+(?:the\s+)?open|ahead\s+of\s+(?:the\s+)?open)\b/i
    : state === 'REGULAR'
      ? /\b(?:regular\s+session|trading(?:\s+(?:today|now))?|during\s+(?:the\s+)?session)\b/i
      : state === 'POST'
        ? /\b(?:post[-\s]?market|after\s+(?:the\s+)?close)\b/i : null;
  return currentMarker.test(firstSentence)
    || Boolean(stateMarker?.test(firstSentence))
    || activeSessionOpeningDatePatterns(input).some(pattern => pattern.test(firstSentence));
}
...
function noCurrentSessionEvidenceOutput(input) {
  if (!isActiveUsAnalysis(input)
      ...
      || input.marketPackages.some(item => item.evidenceContext.evidence.length > 0)) {
    return null;
  }
  return deepFreeze({
    status: 'DEGRADED',
...
function activeSectionOneLeadsCurrentSession(section, input) {
  ...
  return hasActiveSessionOpening(section.content, input);
}
...
function validateClaudeAnalysisOutput(output, input) {
  ...
  const activeUs = isActiveUsAnalysis(input);
  ...
  const currentCatalystReferences = new Set(
  ...
  if (!Array.isArray(output.sections) || output.sections.length !== REPORT_SECTION_NAMES.length) {
    ...
    for (let index = 0; index < output.sections.length; index++) {
      ...
      if (section.content !== null && hasMalformedPlainEnglishProse(section.content)) {
        errors.push(`sections[${index}]: malformed plain-language prose`);
      }
      ...
      } else if (section.uncertainties.some(hasMalformedPlainEnglishProse)) {
        errors.push(`sections[${index}]: malformed plain-language prose in uncertainties`);
      }
      ...
      if (activeWithCurrentEvidence && index === 1 && section.content !== null && validEvidenceRefs
          && !section.evidenceRefs.some(reference => currentDriverReferences.has(reference))) {
        errors.push('sections[1]: active key market drivers require CURRENT_SESSION driver evidence');
      }
      if (index === 1 && section.content !== null && validEvidenceRefs
          && (activeUs
            ? hasActiveDirectMarketCausalClaim(section.content)
      ...
      if (activeUs && index === 1 && section.content !== null && validEvidenceRefs
          && hasActiveDirectMarketCausalClaim(section.content)
          && !hasPriorCompletedSessionCausalClaim(section.content, input)
          && !section.evidenceRefs.some(reference => currentCatalystReferences.has(reference))) {
        errors.push('sections[1]: active market causality requires a CURRENT_SESSION principal catalyst');
      }
      if (index === 1 && section.content !== null && validEvidenceRefs
      ...
      if (activeUs && index !== 1 && index < REPORT_SECTION_NAMES.length - 1
          && section.content !== null && validEvidenceRefs
          && hasActiveDirectMarketCausalClaim(section.content)) {
        const priorClaim = hasPriorCompletedSessionCausalClaim(section.content, input);
        const allowedCatalysts = priorClaim
          ? completedSessionCatalystReferences : currentCatalystReferences;
        if (!section.evidenceRefs.some(reference => allowedCatalysts.has(reference))) {
          errors.push(`sections[${index}]: active market causality lacks the required principal catalyst`);
        }
      }
      ...
      } else if (index === 0 && output.status !== 'FAILED'
          && !isActiveUsAnalysis(input)) {
        errors.push('sections[0]: executive market summary requires supported content');
      }
    }
  }
  ...
  else if (output.evidenceGaps.some(hasMalformedPlainEnglishProse)) {
    errors.push('evidence gaps contain malformed plain-language prose');
  }
...
module.exports = {
  ...
  hasPriorCompletedSessionFirstFraming,
  hasActiveSessionOpening,
