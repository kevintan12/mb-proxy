...
// Context-dependent jargon and broken replacement prose cannot be rewritten without
// risking a changed claim. Reject it through the existing section validator instead.
const PLAIN_ENGLISH_UNSAFE_PATTERNS = Object.freeze([
...
function hasAnalystDeskJargon(value) {
  ...
  return PLAIN_ENGLISH_REPLACEMENTS.some(({pattern}) => new RegExp(pattern, 'i').test(value))
    || PLAIN_ENGLISH_UNSAFE_PATTERNS.some(pattern => pattern.test(value));
}
...
function validateClaudeAnalysisOutput(output, input) {
  ...
  if (!Array.isArray(output.sections) || output.sections.length !== REPORT_SECTION_NAMES.length) {
    ...
    for (let index = 0; index < output.sections.length; index++) {
      ...
      if (section.content !== null && hasAnalystDeskJargon(section.content)) {
        errors.push(`sections[${index}]: analyst jargon requires plain-language wording`);
      }
      ...
      } else if (section.uncertainties.some(hasAnalystDeskJargon)) {
        errors.push(`sections[${index}]: uncertainties require plain-language wording`);
      }
  ...
  else if (output.evidenceGaps.some(hasAnalystDeskJargon)) {
    errors.push('evidence gaps require plain-language wording');
  }
