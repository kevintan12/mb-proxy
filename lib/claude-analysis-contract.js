...
const REPORT_SECTION_PURPOSES = Object.freeze([
  ...
  'Cover material movements and relevant known upcoming events within 14 days for both My Stocks and Watchlist without allowing one unsupported security to collapse the section.',
...
const ANALYSIS_REQUEST_KEYS = Object.freeze([
  'selectedScope', 'generatedAt', 'userTimezone', 'reportType'
]);
...
function normalizeAnalysisRequest(input) {
  ...
  if (!SELECTED_SCOPES.includes(selectedScope) || !generatedAt || !userTimezone
      || !REPORT_TYPES.includes(reportType)) return null;
  return {selectedScope, generatedAt, userTimezone, reportType};
}
...
function validateClaudeAnalysisOutput(output, input) {
  ...
  if (output.status === 'NORMAL') {
    if (supportedSections !== analysisSections.length) errors.push('NORMAL requires every analysis section');
