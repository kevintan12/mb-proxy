...
function canonicalArticle(article, bounds) {
  const deterministicIdentity = deterministicCnbcRecapCandidate(article?.targetSessionDate)?.url
  ...
  if (!hasExactKeys(article, CNBC_RECAP_ARTICLE_CONTENT_RESULT_KEYS)
      ...
      || (!deterministicIdentity && !canonicalTimestamp(article.publishedAt))
      || (!deterministicIdentity && article.updatedAt !== null
        && !canonicalTimestamp(article.updatedAt))
      || !canonicalDate(article.targetSessionDate)
      || (!deterministicIdentity
        && newYorkDate(article.publishedAt) !== article.targetSessionDate
        && dailyRecapSessionDate(article.title) !== article.targetSessionDate)
...
function createCnbcRecapEvidenceConstructionService({evidenceConstructionBounds} = {}) {
  ...
  return Object.freeze({
    constructEvidence({articleContent, horizon, observedAt} = {}) {
      ...
      const deterministicIdentity = deterministicCnbcRecapCandidate(article.targetSessionDate)?.url
        === article.canonicalUrl;
      const canonicalEvidenceHorizon = deterministicIdentity
        ? null : canonicalHorizon(horizon, article.publishedAt, article.updatedAt);
      if (!deterministicIdentity && !canonicalEvidenceHorizon) {
        return failure('INPUT_FAILURE', 'Invalid CNBC recap evidence horizon');
      }
      let evidenceItem;
      const providerPublishedAt = canonicalTimestamp(article.publishedAt);
      const cutoff = canonicalTimestamp(observedAt);
      const evidencePublishedAt = deterministicIdentity
        ? providerPublishedAt && cutoff && Date.parse(providerPublishedAt) <= Date.parse(cutoff)
      ...
      try {
        evidenceItem = createEvidenceItem({
          ...
          publishedAt: evidencePublishedAt, symbols: []
        });
      } catch (error) {
      ...
      const publishedAt = canonicalTimestamp(article.publishedAt);
      const updatedAt = canonicalTimestamp(article.updatedAt);
      const constructedEvidence = {
        targetSessionDate: article.targetSessionDate,
        updatedAt: deterministicIdentity
