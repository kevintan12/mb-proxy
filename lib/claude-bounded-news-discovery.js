...
function selectDiscovery(searchResults, targetSessionDate) {
  const seen = new Set();
  const inspected = searchResults.slice(0, CLAUDE_BOUNDED_NEWS_DISCOVERY_MAX_RESULTS_INSPECTED);
  for (const result of inspected) {
    const url = canonicalYahooRecapUrl(result.url);
    const title = canonicalTitle(result.title);
    if (!url || !title || seen.has(url)) continue;
    seen.add(url);
    return deepFreeze({title, url, discoveredVia: DISCOVERED_VIA, targetSessionDate});
  }
  return null;
}
...
function createClaudeBoundedNewsDiscoveryService({
  ...
  return deepFreeze({
    async discoverYahooCompletedSessionRecap(context) {
      ...
      const discovery = selectDiscovery(parsed.results, canonical.targetSessionDate);
      finishDiagnostics(requestId, envelope?.usage);
      return discovery
