...
function buildClaudeBoundedNewsDiscoveryRequest(context) {
  ...
  const searchIntent = `Yahoo US stock market today ${ENGLISH_MONTH_NAMES[month - 1]} ${day} ${year}`;
  return deepFreeze({
  ...
  return deepFreeze({
    async discoverYahooCompletedSessionRecap(context) {
      ...
      return candidates.length > 0
        ? deepFreeze({ok: true, type: 'SUCCESS', candidates})
        : deepFreeze({ok: true, type: 'NOT_FOUND', candidates});
    }
  });
}
