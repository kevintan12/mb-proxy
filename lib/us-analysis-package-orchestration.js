...
const ACTIVE_SESSION_STATES = new Set(['PRE', 'REGULAR', 'POST']);
const COMPLETED_SESSION_STATES = new Set(['CLOSED', 'WEEKEND', 'HOLIDAY']);
const ACTIVE_YAHOO_MAX_ARTICLE_ATTEMPTS = 5;
...
function headlineMatchesMostActive(candidate, mostActive) {
  const headline = typeof candidate?.headline === 'string' ? candidate.headline.toLowerCase() : '';
  return mostActive.some(item => {
    ...
    return new RegExp(`(^|[^a-z0-9])${symbol.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^a-z0-9]|$)`, 'i')
      .test(headline) || names.some(name => headline.includes(name));
  });
}
...
function selectedActiveYahooCandidates(mostActive, candidates) {
  ...
  return Object.freeze(matched.concat(remaining).slice(0, ACTIVE_YAHOO_MAX_ARTICLE_ATTEMPTS));
}

function activeYahooSymbols(headline, mostActive) {
  const value = typeof headline === 'string' ? headline.toLowerCase() : '';
  return mostActive.filter(item => {
    const escaped = item.symbol.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return new RegExp(`(^|[^a-z0-9])${escaped}([^a-z0-9]|$)`, 'i').test(value)
      || [item.shortName, item.longName].some(name =>
        typeof name === 'string' && name.trim().length >= 3 && value.includes(name.toLowerCase()));
  ...
  async function acquireAnalysisMaterial(context, benchmarkAnchors, stageTiming, setAssemblyFailureStage) {
    ...
    if (analysisMode === ANALYSIS_MODES.ACTIVE_SESSION) {
      ...
      for (const candidate of selected) {
        ...
        if (result?.ok !== true || result.type !== 'SUCCESS' || !result.articleContent) continue;
        ...
        if (typeof article.publishedAt !== 'string'
            ...
            || Date.parse(article.publishedAt) > Date.parse(context.acquisitionStartedAt)
            || seenUrls.has(article.canonicalUrl)) continue;
        try {
          const item = createEvidenceItem({
            ...
            symbols: activeYahooSymbols(article.headline, mostActive),
    ...
    for (const item of acquiredActiveYahooItems) {
      ...
      const trialHorizons = requiredBaseEvidenceHorizons.concat(
        activeYahooItems.map(() => 'SUBSEQUENT_DEVELOPMENT'),
      ...
      marketPackages: [{
        ...
        evidenceContext: {
          ...
          subsequentDevelopments: subsequentYahooRecapReferences.concat(
            subsequentCnbcRecapReferences, activeYahooReferences, subsequentCnbcReferences
          ),
