...
function createUsAnalysisPackageOrchestrationService({
  ...
  async function acquireAnalysisMaterial(context, benchmarkAnchors, stageTiming, setAssemblyFailureStage) {
    ...
    const evidenceItems = yahooItems.concat(
      yahooRecapItems, federalReserveItems, cnbcRecapItems, cnbcItems
    );
    ...
    const evidenceHorizons = [
      ...
      ...cnbcConstructedEvidence.map(record => record.horizon.classification)
    ];
    const cnbcSessionAssociations = cnbcRecapConstructedEvidence
      ...
      : [];
    const classificationInput = Object.freeze({
      ...
      evidence: Object.freeze(evidenceCollection.items.map((item, index) => Object.freeze({
        reference: `e${index + 1}`,
        horizon: evidenceHorizons[index],
        item
      })))
    });
    let materialEvents = [];
    ...
    return {
      marketPackages: [{
        ...
        evidenceContext: {
          ...
          unresolvedGaps,
          furtherReadings: yahooRecapReferences.concat(cnbcRecapReferences).map(evidenceRef => ({
            evidenceRef,
