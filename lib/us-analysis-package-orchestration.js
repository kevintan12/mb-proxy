...
function createUsAnalysisPackageOrchestrationService({
  ...
  async function acquireAnalysisMaterial(context, benchmarkAnchors, stageTiming, setAssemblyFailureStage) {
    ...
    function classificationInputFor(items, horizons, broadMarketEvidenceStartIndex) {
      ...
      return Object.freeze({
        ...
        evidence: Object.freeze(collection.items.map((item, index) => Object.freeze({
          ...
          requiresBroadMarketSubjects: index >= broadMarketEvidenceStartIndex,
          item
        })))
      });
    }
      ...
      for (const record of cnbcConstructedEvidence) {
        ...
        try {
          const request = buildClaudeEvidenceRoleClassificationRequest(
            classificationInputFor(trialItems, trialHorizons, baseEvidenceItems.length)
          );
    ...
    const provisionalEvidenceCollection = createEvidenceCollection({
      market: 'US', items: provisionalEvidenceItems
    });
    const classificationInput = classificationInputFor(
      provisionalEvidenceCollection.items,
      provisionalEvidenceHorizons,
      baseEvidenceItems.length
    );
    ...
    if (marketContext.primaryCompletedSessionDate !== null) {
      ...
      const classificationsByReference = new Map(
        output.classifications.map(classification => [classification.reference, classification])
      );
      ...
      finalClassifications = output.classifications.flatMap(classification => {
    ...
    broadMarketFocus = (cnbcClassifierCapacityUnavailable ? [] : finalClassifications)
      .filter(classification => {
        const item = finalEvidenceByReference.get(classification.reference);
        return item?.evidenceCategory === 'news'
