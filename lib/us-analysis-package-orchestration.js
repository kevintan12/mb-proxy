const {createAnalysisPackageService} = require('./analysis-package-service');
...
function createUsAnalysisPackageOrchestrationService({
  ...
  async function acquireAnalysisMaterial(context, benchmarkAnchors) {
    ...
    for (const symbol of acquisitionSymbols) {
      const acquired = requireSnapshot(
        await telemetryAcquisition.acquireSnapshot({market: 'US', symbol}),
        symbol
      );
      const persisted = requireSnapshot(await snapshotPersistence.persistSnapshot(acquired), symbol);
      snapshots.push(persisted);
      yahooCollections.push(requireEvidenceCollection(
        await yahooEvidenceAcquisition.acquireEvidence({symbol}),
    ...
    try {
      federalReserveCollection = requireEvidenceCollection(
        await federalReserveEvidenceAcquisition.acquireEvidence(),
  ...
  return Object.freeze({
    async assemble(input) {
      ...
      const packageService = createAnalysisPackageService({
        acquireAnalysisMaterial: context => acquireAnalysisMaterial(context, benchmarkAnchors),
        now
      });
      return packageService.assemble(canonicalRequest);
    }
  });
}
