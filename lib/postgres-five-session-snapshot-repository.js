...
function normalizeKey(input) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new TypeError('five-session snapshot repository key must be an object');
  }
...
function reconstructRows(result, market, symbol) {
  const rows = result && Array.isArray(result.rows) ? result.rows.slice(-5) : [];
...
function createPostgresFiveSessionSnapshotRepository({query, transaction} = {}) {
  ...
  return Object.freeze({
    ...
    async upsertSnapshot({market: inputMarket, symbol: inputSymbol, sessions} = {}) {
      ...
      if (!Array.isArray(sessions) || sessions.length > 5) {
        throw new TypeError('sessions must be an array containing at most five canonical sessions');
      }
