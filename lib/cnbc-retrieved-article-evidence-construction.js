...
const PROVISIONAL_EVIDENCE_RECORD_KEYS = Object.freeze([
  'candidateReference',
  'horizon',
  'evidenceItem'
]);
...
function provisionalFailure(type, message, failedCandidateReferences = []) {
  return deepFreeze({
    ok: false,
    type,
    message,
    failedCandidateReferences: failedCandidateReferences.slice()
  });
}
...
function createCnbcRetrievedArticleEvidenceConstructionService({
  ...
  return Object.freeze({
    constructProvisionalEvidence({candidateCollection, retrievedArticles} = {}) {
      ...
      try {
        ...
        return failure('INPUT_FAILURE', 'Invalid CNBC candidate collection');
      }
      if (!Array.isArray(retrievedArticles)
          || retrievedArticles.length !== canonicalCollection.candidates.length) {
        return provisionalFailure(
          'ARTICLE_VALIDATION_FAILURE',
          'CNBC retrieved articles do not match candidates'
        );
      }

      const evidenceInputs = [];
      for (let index = 0; index < canonicalCollection.candidates.length; index++) {
        const candidate = canonicalCollection.candidates[index];
        try {
          validateRetrievedArticle(retrievedArticles[index], candidate);
        } catch (error) {
          return provisionalFailure(
            'ARTICLE_VALIDATION_FAILURE',
            'CNBC retrieved articles do not match candidates',
        ...
        if (Buffer.byteLength(candidate.title, 'utf8') > constructionLimits.maxTitleBytes
            || Buffer.byteLength(candidate.extract, 'utf8')
              > constructionLimits.maxEvidenceTextBytes) {
          return provisionalFailure(
            'EVIDENCE_TOO_LARGE',
      ...
      try {
        ...
        return provisionalFailure(
          'EVIDENCE_CONTRACT_FAILURE',
          'CNBC evidence construction failed canonical validation',
          canonicalCollection.candidates.map(candidate => candidate.reference)
        );
      }
      const constructedEvidence = evidenceCollection.items.map((evidenceItem, index) => ({
        candidateReference: canonicalCollection.candidates[index].reference,
        horizon: {...canonicalCollection.candidates[index].horizon},
        evidenceItem
      }));
      if (constructedEvidence.some(record =>
        !hasExactKeys(record, PROVISIONAL_EVIDENCE_RECORD_KEYS))
...
module.exports = {
  ...
  CNBC_PROVISIONAL_EVIDENCE_RECORD_KEYS: PROVISIONAL_EVIDENCE_RECORD_KEYS,
