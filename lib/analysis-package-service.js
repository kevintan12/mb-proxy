...
const FUTURE_EVIDENCE_OMITTED_GAP =
  'Future-dated evidence was omitted because it was unavailable at report generation time.';
const EVIDENCE_ROLE_KEYS = Object.freeze([
...
function omitFutureUsEvidence(material, generatedAt, onDiagnostics) {
  const marketPackage = material.marketPackages[0];
  if (marketPackage?.market !== 'US') return material;
  const originalItems = marketPackage.evidenceCollection.items;
  ...
  for (let index = 0; index < originalItems.length; index++) {
    ...
    if (item.publishedAt !== null && Date.parse(item.publishedAt) > cutoff) {
      omittedReferences.add(`e${index + 1}`);
      continue;
    }
    referenceMap.set(`e${index + 1}`, `e${keptItems.length + 1}`);
    keptItems.push(item);
  }
  const omittedCount = originalItems.length - keptItems.length;
  if (omittedCount === 0) return material;
  const knownReference = reference => referenceMap.has(reference)
    || omittedReferences.has(reference);
  const context = marketPackage.evidenceContext;
  const usedReferences = EVIDENCE_ROLE_KEYS.flatMap(key => context[key])
    .concat(context.sessionAssociations.map(entry => entry.evidenceRef),
      context.broadMarketFocus.map(entry => entry.evidenceRef),
      context.furtherReadings.map(entry => entry.evidenceRef),
      material.portfolioContext.myStocks.concat(material.portfolioContext.watchlist)
        .flatMap(security => security.evidenceRefs.concat(
          security.upcomingEvents.flatMap(event => event.evidenceRefs))));
  if (usedReferences.some(reference => !knownReference(reference))) {
    throw new TypeError('invalid evidence reference in acquired material');
  }
  const mapped = references => references.flatMap(reference =>
    omittedReferences.has(reference) ? [] : [referenceMap.get(reference) || reference]);
  const evidenceContext = {...context};
  for (const key of EVIDENCE_ROLE_KEYS) evidenceContext[key] = mapped(context[key]);
  evidenceContext.sessionAssociations = context.sessionAssociations.flatMap(association =>
    omittedReferences.has(association.evidenceRef) ? []
      : [{...association, evidenceRef:
        referenceMap.get(association.evidenceRef) || association.evidenceRef}]);
  evidenceContext.broadMarketFocus = context.broadMarketFocus.flatMap(entry =>
    omittedReferences.has(entry.evidenceRef) ? []
      : [{...entry, evidenceRef: referenceMap.get(entry.evidenceRef) || entry.evidenceRef}]);
  evidenceContext.furtherReadings = context.furtherReadings.flatMap(reading =>
    omittedReferences.has(reading.evidenceRef) ? []
      : [{...reading, evidenceRef: referenceMap.get(reading.evidenceRef) || reading.evidenceRef}]);
  evidenceContext.unresolvedGaps = context.unresolvedGaps.includes(FUTURE_EVIDENCE_OMITTED_GAP)
    ? context.unresolvedGaps.slice() : [...context.unresolvedGaps, FUTURE_EVIDENCE_OMITTED_GAP];
  function mapPortfolioList(list) {
    return list.map(security => ({
      ...security,
      evidenceRefs: mapped(security.evidenceRefs),
      upcomingEvents: security.upcomingEvents.flatMap(event => {
        const evidenceRefs = mapped(event.evidenceRefs);
        return evidenceRefs.length ? [{...event, evidenceRefs}] : [];
      })
    }));
  }
  if (typeof onDiagnostics === 'function') {
    try {
      onDiagnostics(deepFreeze({
        stage: 'futureEvidenceOmission', omittedReferenceCount: omittedCount,
        causalClassificationDiscarded: ['materialEvents', 'principalCatalysts',
          'conflictingEvidence'].some(key => context[key].some(reference =>
          omittedReferences.has(reference)))
      }));
    } catch (error) {
      // Observability must not alter package assembly.
    }
  }
  return {
    marketPackages: [{
      ...marketPackage,
      evidenceCollection: {...marketPackage.evidenceCollection, items: keptItems},
      evidenceContext
    }],
    portfolioContext: {
      myStocks: mapPortfolioList(material.portfolioContext.myStocks),
      watchlist: mapPortfolioList(material.portfolioContext.watchlist)
    }
  };
}
  ...
  return Object.freeze({
    async assemble(request) {
      ...
      try {
        ...
        let material = await acquireAnalysisMaterial(acquisitionContext, setFailureStage);
        ...
        if (canonicalRequest.selectedScope === 'US') {
          material = omitFutureUsEvidence(material, generatedAt, onDiagnostics);
          validateAcquiredMaterial(material, canonicalRequest, markets);
        }
