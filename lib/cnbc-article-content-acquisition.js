...
const CNBC_SESSION_MISMATCH_DIAGNOSTIC_TYPES = new Set([
  'HEADLINE_SESSION_MISMATCH',
  'PUBLICATION_BEFORE_TARGET_SESSION',
...
function inspectCnbcStateArticle(html, expectedUrl) {
  ...
  if (!headline || !articleText) return {extracted: null, failureType: 'NO_USABLE_BODY'};
  ...
  if (header.data.dateModified !== undefined && header.data.dateModified !== null && !updatedAt) {
    return {extracted: null, failureType: 'INVALID_DATE_MODIFIED'};
  }
  return {
    extracted: {
...
function extractArticle(html, expectedUrl) {
  const scripts = [];
  ...
  const stateArticle = conventional.extracted || liveBlog.extracted
    ? null : inspectCnbcStateArticle(html, expectedUrl);
...
function createCnbcArticleContentAcquisitionService({fetchImpl = global.fetch, onDiagnostics} = {}) {
  ...
  function extractWithDiagnostics(html, operation, discoveryRank = null, expectedUrl = null) {
    try {
      return extractArticle(html, expectedUrl);
    } catch (error) {
  ...
  return Object.freeze({
    ...
    async acquireRecapArticleContent({discovery, bounds} = {}) {
      ...
      const html = await fetchArticleHtml(fetchImpl, canonicalUrl, limits);
      const extracted = extractWithDiagnostics(html, 'RECAP', null, canonicalUrl);
      const publicationMatchesSession =
        exchangeDate(extracted.publishedAt) === discovery.targetSessionDate;
      const editorialSessionDate = dailyRecapSessionDate(extracted.editorialHeadline);
      const deterministicHeadlineMismatch = editorialSessionDate !== discovery.targetSessionDate;
      const deterministicPublicationBeforeTarget = exchangeDate(extracted.publishedAt) < discovery.targetSessionDate;
      const sessionMismatch = discovery.discoveredVia === CNBC_DETERMINISTIC_RECAP_DISCOVERED_VIA
        ? deterministicHeadlineMismatch || deterministicPublicationBeforeTarget
        : !publicationMatchesSession && editorialSessionDate !== discovery.targetSessionDate;
      if (sessionMismatch) {
        const sessionMismatchType = discovery.discoveredVia === CNBC_DETERMINISTIC_RECAP_DISCOVERED_VIA
          ? (deterministicHeadlineMismatch
            ? 'HEADLINE_SESSION_MISMATCH' : 'PUBLICATION_BEFORE_TARGET_SESSION')
          : 'PUBLICATION_OR_EDITORIAL_SESSION_MISMATCH';
        throw fail('SESSION_MISMATCH', 'CNBC recap publication date does not match the target session',
          undefined, undefined, undefined, sessionMismatchType);
      }
      ...
      const title = discovery.discoveredVia === CNBC_DETERMINISTIC_RECAP_DISCOVERED_VIA
        || !publicationMatchesSession ? extracted.editorialHeadline : discovery.title;
