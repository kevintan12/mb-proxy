...
function extractableArticle(node) {
  ...
  if (!articleText || !publishedAt
      || (node.dateModified !== undefined && node.dateModified !== null && !updatedAt)) return null;
  return {node, articleText, publishedAt, updatedAt};
}
...
function firstLiveBlogUpdate(nodes) {
  for (const node of nodes) {
    ...
    for (const update of node.liveBlogUpdate) {
      const extracted = hasType(update, BLOG_POST_TYPE) ? extractableArticle(update) : null;
...
function extractArticle(html) {
  ...
  const extracted = articles.map(extractableArticle).find(Boolean)
    || firstLiveBlogUpdate(liveBlogs);
  if (!extracted) throw fail('EXTRACTION_FAILURE', 'CNBC article content or timestamps could not be extracted');
