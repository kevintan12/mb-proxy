...
class CnbcArticleContentAcquisitionError extends Error {
  constructor(code, message, sizeFailureType, extractionFailureType, extractionDiagnostics) {
    super(message);
...
function fail(code, message, sizeFailureType, extractionFailureType, extractionDiagnostics) {
  return new CnbcArticleContentAcquisitionError(
    code, message, sizeFailureType, extractionFailureType, extractionDiagnostics
  );
}
...
function createCnbcArticleContentAcquisitionService({fetchImpl = global.fetch, onDiagnostics} = {}) {
  ...
  return Object.freeze({
    ...
    async acquireRecapArticleContent({discovery, bounds} = {}) {
      ...
      if ((discovery.discoveredVia === CNBC_DETERMINISTIC_RECAP_DISCOVERED_VIA
          && (editorialSessionDate !== discovery.targetSessionDate
            || exchangeDate(extracted.publishedAt) < discovery.targetSessionDate))
          || (discovery.discoveredVia !== CNBC_DETERMINISTIC_RECAP_DISCOVERED_VIA
