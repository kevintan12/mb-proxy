...
function anthropicDiscoveryResponse(content, usage = {
  ...
  return {
    ok: true,
    status: 200,
    headers: {get: name => name === 'request-id' ? 'req_cnbc_recap_123' : null},
...
test('temporary CNBC recap discovery rejects wrong methods and unauthorized requests before provider work', async () => {
  const previousToken = process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
  let fetches = 0;
  global.fetch = async () => { fetches++; throw new Error('must not run'); };
  try {
    process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = 'expected';
    const wrongMethod = mockRes();
    await handler({method: 'GET', query: {cnbcRecapDiscoveryValidation: '1'}, headers: {}, body: {}}, wrongMethod);
    ...
    for (const testCase of [
      ...
      if (testCase.environment === undefined) delete process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
      else process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = testCase.environment;
      const res = mockRes();
      await handler({
        method: 'POST', query: {cnbcRecapDiscoveryValidation: '1'},
    ...
    if (previousToken === undefined) delete process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
    else process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = previousToken;
  }
});

test('temporary CNBC recap discovery rejects malformed, missing, and expanded input before provider work', async () => {
  const previousToken = process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
  process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = 'validation-token';
  let fetches = 0;
  global.fetch = async () => { fetches++; throw new Error('must not run'); };
  try {
    for (const body of [
      ...
      {targetSessionDate: '2026-09-11', model: 'caller'},
      {targetSessionDate: '2026-09-11', tools: []},
      {targetSessionDate: '2026-09-11', allowed_domains: ['example.com']},
      {targetSessionDate: '2026-09-11', max_uses: 99},
      {targetSessionDate: '2026-09-11', resultCeiling: 99},
      {targetSessionDate: '2026-09-11', retry: true},
      {targetSessionDate: '2026-09-11', prompt: 'caller'},
      {targetSessionDate: '2026-09-11', url: 'https://www.cnbc.com/'}
    ]) {
      const res = mockRes();
      await handler({
        method: 'POST', query: {cnbcRecapDiscoveryValidation: '1'},
...
test('temporary CNBC recap discovery returns one sanitized FOUND result without other runtime work', async () => {
  const previousToken = process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
  const previousKey = process.env.ANTHROPIC_API_KEY;
  const originalGetter = analysisPackageRuntime.getAnalysisPackageRuntime;
  process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = 'validation-token';
  process.env.ANTHROPIC_API_KEY = 'anthropic-secret';
  let packageRuntimeGets = 0;
  analysisPackageRuntime.getAnalysisPackageRuntime = () => { packageRuntimeGets++; throw new Error('must not run'); };
  const calls = [];
  global.fetch = async (url, options) => {
    ...
    return anthropicDiscoveryResponse([{
      ...
      content: [{
        ...
        title: 'Stock market today: Wall Street closes higher',
        url: 'https://www.cnbc.com/2026/09/11/stock-market-today-live-updates.html?tracking=1#fragment',
        encrypted_content: 'private provider content'
      }]
  ...
  try {
    ...
    await handler({
      ...
      body: {targetSessionDate: '2026-09-11'}
    }, res);
    assert.equal(res.statusCode, 200);
    assert.equal(calls.length, 1);
    assert.equal(calls[0].url, 'https://api.anthropic.com/v1/messages');
    assert.equal(packageRuntimeGets, 0);
    assert.deepEqual(res.body.outcome, {
      type: 'FOUND',
      targetSessionDate: '2026-09-11',
      title: 'Stock market today: Wall Street closes higher',
      url: 'https://www.cnbc.com/2026/09/11/stock-market-today-live-updates.html',
      discoveredVia: 'ANTHROPIC_WEB_SEARCH'
    });
    assert.deepEqual(Object.keys(res.body.diagnostics), [
      'requestId', 'requestBytes', 'elapsedMs', 'usage',
      'searchRequestCount', 'fetchCount', 'observedBlockTypes'
    ]);
    assert.equal(res.body.diagnostics.requestId, 'req_cnbc_recap_123');
    ...
    for (const forbidden of [
      'validation-token', 'anthropic-secret', 'private query',
      'private provider content', 'private narrative', 'tracking=1', 'fragment'
    ]) assert.equal(serialized.includes(forbidden), false);
    assert.equal(Object.hasOwn(res.body.outcome, 'publishedAt'), false);
    assert.equal(Object.hasOwn(res.body.outcome, 'articleText'), false);
    assert.equal(Object.hasOwn(res.body.outcome, 'evidenceRef'), false);
  } finally {
    analysisPackageRuntime.getAnalysisPackageRuntime = originalGetter;
    if (previousToken === undefined) delete process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
    else process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = previousToken;
...
test('temporary CNBC recap discovery returns sanitized NOT_FOUND and tool failure outcomes', async () => {
  const previousToken = process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
  const previousKey = process.env.ANTHROPIC_API_KEY;
  process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = 'validation-token';
  process.env.ANTHROPIC_API_KEY = 'anthropic-secret';
  try {
    ...
    global.fetch = async () => {
      ...
      return anthropicDiscoveryResponse([{
        type: 'web_search_tool_result', content: [{
          type: 'web_search_result', title: 'Other CNBC story',
          url: 'https://www.cnbc.com/2026/09/11/unrelated.html'
        }]
      }]);
    };
    const notFound = mockRes();
    await handler({method: 'POST', query: {cnbcRecapDiscoveryValidation: '1'},
      headers: {authorization: 'Bearer validation-token'}, body: {targetSessionDate: '2026-09-11'}}, notFound);
    assert.equal(notFound.statusCode, 200);
    assert.deepEqual(notFound.body.outcome, {type: 'NOT_FOUND', targetSessionDate: '2026-09-11'});
    assert.equal(JSON.stringify(notFound.body).includes('Other CNBC story'), false);
    ...
    global.fetch = async () => {
      ...
      return anthropicDiscoveryResponse([{
        ...
        content: {type: 'web_search_tool_result_error', error_code: 'private-code'}
      }]);
    };
    const failure = mockRes();
    await handler({method: 'POST', query: {cnbcRecapDiscoveryValidation: '1'},
      headers: {authorization: 'Bearer validation-token'}, body: {targetSessionDate: '2026-09-11'}}, failure);
    assert.equal(failure.statusCode, 502);
    assert.deepEqual(failure.body.outcome, {type: 'SEARCH_TOOL_FAILURE', targetSessionDate: '2026-09-11'});
    assert.equal(JSON.stringify(failure.body).includes('private-code'), false);
...
test('temporary CNBC recap discovery fails safely when the Anthropic key is absent', async () => {
  const previousToken = process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
  const previousKey = process.env.ANTHROPIC_API_KEY;
  process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = 'validation-token';
  ...
  try {
    const res = mockRes();
    await handler({method: 'POST', query: {cnbcRecapDiscoveryValidation: '1'},
      headers: {authorization: 'Bearer validation-token'}, body: {targetSessionDate: '2026-09-11'}}, res);
    assert.equal(res.statusCode, 502);
    assert.deepEqual(res.body, {
      outcome: {type: 'UPSTREAM_FAILURE', targetSessionDate: '2026-09-11'},
      diagnostics: {
        requestId: null, requestBytes: null, elapsedMs: null, usage: {},
        searchRequestCount: null, fetchCount: 0, observedBlockTypes: []
      }
    });
    assert.equal(fetches, 0);
    assert.equal(JSON.stringify(res.body).includes('validation-token'), false);
  } finally {
    if (previousToken === undefined) delete process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN;
    else process.env.CNBC_RECAP_DISCOVERY_VALIDATION_TOKEN = previousToken;
    if (previousKey === undefined) delete process.env.ANTHROPIC_API_KEY;
